...
(function(){
  ...
  async function finalSaveEditor(editing, isNew, originalName){
    ...
    try {
      ...
      const scaleNumber = (id, fallback=100) => {
        const n = Number(val(id));
        return Number.isFinite(n) ? Math.max(40, Math.min(160, n)) : fallback;
      };
      if (!build.imageScales) build.imageScales = {};
      editing.cardImageScale = scaleNumber("scaleCardImage", editing.cardImageScale || 100);
      Object.assign(build.imageScales, {
        fashionImage:scaleNumber("scaleFashionImage", build.imageScales.fashionImage || 100),
        buildImage:scaleNumber("scaleBuildImage", build.imageScales.buildImage || 100),
        special0Image:scaleNumber("scaleSpecialImage_0", build.imageScales.special0Image || 100),
        special0Build:scaleNumber("scaleSpecialBuild_0", build.imageScales.special0Build || 100),
        special1Image:scaleNumber("scaleSpecialImage_1", build.imageScales.special1Image || 100),
        special1Build:scaleNumber("scaleSpecialBuild_1", build.imageScales.special1Build || 100),
        special2Image:scaleNumber("scaleSpecialImage_2", build.imageScales.special2Image || 100),
        special2Build:scaleNumber("scaleSpecialBuild_2", build.imageScales.special2Build || 100),
        primaryImage:scaleNumber("scalePrimaryImage", build.imageScales.primaryImage || 100),
        primaryBuild:scaleNumber("scalePrimaryBuild", build.imageScales.primaryBuild || 100),
        secondaryImage:scaleNumber("scaleSecondaryImage", build.imageScales.secondaryImage || 100),
        secondaryBuild:scaleNumber("scaleSecondaryBuild", build.imageScales.secondaryBuild || 100),
        meleeImage:scaleNumber("scaleMeleeImage", build.imageScales.meleeImage || 100),
        meleeBuild:scaleNumber("scaleMeleeBuild", build.imageScales.meleeBuild || 100),
        companionSkinImage:scaleNumber("scaleCompanionSkinImage", build.imageScales.companionSkinImage || 100),
        companionImage:scaleNumber("scaleCompanionImage", build.imageScales.companionImage || 100),
        companionBuild:scaleNumber("scaleCompanionBuild", build.imageScales.companionBuild || 100),
        companionWeaponImage:scaleNumber("scaleCompanionWeaponImage", build.imageScales.companionWeaponImage || 100),
        companionWeaponBuild:scaleNumber("scaleCompanionWeaponBuild", build.imageScales.companionWeaponBuild || 100)
      });

      const uploads = [
        ["editCardImage","cards", u => editing.cardImage = u],
  ...
  function forceFloatingControlsFinal(){
    ...
    if (music) {
      Object.assign(music.style, {
        position:'fixed', top:'calc(env(safe-area-inset-top, 0px) + 16px)', left:'calc(env(safe-area-inset-left, 0px) + 16px)',
        zIndex:'1000000', opacity:'1', visibility:'visible', pointerEvents:'auto'
      });
    }
    if (admin) {
      Object.assign(admin.style, {
        position:'fixed', top:'calc(env(safe-area-inset-top, 0px) + 16px)', right:'calc(env(safe-area-inset-right, 0px) + 16px)',
        zIndex:'1000001', opacity:'1', visibility:'visible', pointerEvents:'auto', display:'flex'
      });
    }
  }
...
/* ---------- PATCH FINAL UTILISATEUR : déconnexion sans œil, flèche retour, échelles retirées, boutons fixes ---------- */
(function(){
  function byId(id){ return document.getElementById(id); }

  function removeScaleControls(){
    document.querySelectorAll(
      '.minch-scale-box, .minch-scale-label, .minch-scale-range, .minch-scale-value, [data-scale-for], ' +
      '#editScaleSkinWarframe, #editScaleConfigWarframe, ' +
      '#scaleCardImage, #scaleFashionImage, #scaleBuildImage, ' +
      '#scaleSpecialImage_0, #scaleSpecialBuild_0, #scaleSpecialImage_1, #scaleSpecialBuild_1, #scaleSpecialImage_2, #scaleSpecialBuild_2, ' +
      '#scalePrimaryImage, #scalePrimaryBuild, #scaleSecondaryImage, #scaleSecondaryBuild, #scaleMeleeImage, #scaleMeleeBuild, ' +
      '#scaleCompanionSkinImage, #scaleCompanionImage, #scaleCompanionBuild, #scaleCompanionWeaponImage, #scaleCompanionWeaponBuild'
    ).forEach(function(el){ el.remove(); });
  }

  function forceFixedTopButtons(){
    const music = byId('musicPanel');
    const admin = byId('adminButton');

    if (music) {
      music.style.setProperty('position', 'fixed', 'important');
      music.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 16px)', 'important');
      music.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 16px)', 'important');
      music.style.setProperty('z-index', '10000000', 'important');
    ...
    if (admin) {
      ...
      admin.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 16px)', 'important');
      admin.style.setProperty('right', 'calc(env(safe-area-inset-right, 0px) + 16px)', 'important');
      admin.style.setProperty('z-index', '10000001', 'important');
      ...
      admin.style.setProperty('transform', 'none', 'important');
      admin.style.setProperty('display', 'flex', 'important');
    }
  }

  function bindBackArrows(){
    const buildBack = document.querySelector('#buildPage .back');
    if (buildBack) {
      buildBack.style.setProperty('display', 'flex', 'important');
      buildBack.style.setProperty('position', 'fixed', 'important');
      buildBack.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 18px)', 'important');
      buildBack.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 18px)', 'important');
      buildBack.style.setProperty('z-index', '10000002', 'important');
      buildBack.onclick = function(event){
        event.preventDefault();
        event.stopPropagation();
        if (typeof closeBuild === 'function') closeBuild();
    ...
    document.querySelectorAll('#adminEditorModal .admin-back, #adminLoginModal .admin-back, .admin-back').forEach(function(btn){
      btn.style.setProperty('display', 'flex', 'important');
      btn.style.setProperty('position', 'fixed', 'important');
      btn.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 18px)', 'important');
      btn.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 18px)', 'important');
      btn.style.setProperty('z-index', '10000002', 'important');
      btn.onclick = function(event){
        ...
        if (login && login.classList.contains('open') && typeof closeAdminLogin === 'function') closeAdminLogin();
      };
    });
  }

  function setAdminLogoutView(){
    const modal = byId('adminLoginModal');
    if (!modal) return;

    const title = modal.querySelector('.admin-login-card h2');
    const hint = modal.querySelector('.admin-hint');
    const user = byId('adminUser');
    const pass = byId('adminPass');
    ...
    if (window.isAdminMode) {
      ...
      [user, pass, passWrap, eye].forEach(function(el){
        if (!el) return;
      ...
      if (submit) {
        ...
        submit.onclick = function(event){
          ...
          if (typeof logoutAdmin === 'function') logoutAdmin();
          if (typeof closeAdminLogin === 'function') closeAdminLogin();
          setTimeout(applyFinalUserPatch, 0);
        };
      }
      return;
    }
    ...
    [user, pass, passWrap, eye].forEach(function(el){
      ...
      el.style.setProperty('display', el === eye ? 'flex' : 'block', 'important');
    });
    if (submit) {
      submit.textContent = 'Se connecter';
      submit.classList.remove('admin-centered-logout');
      submit.onclick = function(event){
        ...
        event.stopPropagation();
        if (typeof tryAdminLogin === 'function') tryAdminLogin();
        setTimeout(applyFinalUserPatch, 40);
      };
    }
  }

  function applyFinalUserPatch(){
    removeScaleControls();
    forceFixedTopButtons();
    bindBackArrows();
    setAdminLogoutView();
  }

  const previousOpenAdminLogin = window.openAdminLogin || (typeof openAdminLogin !== 'undefined' ? openAdminLogin : null);
  if (previousOpenAdminLogin && !previousOpenAdminLogin.__minchUserFinalPatch) {
    const patchedOpenAdminLogin = function(){
      const result = previousOpenAdminLogin.apply(this, arguments);
      setTimeout(applyFinalUserPatch, 0);
      setTimeout(applyFinalUserPatch, 80);
      return result;
    };
    patchedOpenAdminLogin.__minchUserFinalPatch = true;
  ...
  if (previousOpenAdminEditor && !previousOpenAdminEditor.__minchUserFinalPatch) {
    const patchedOpenAdminEditor = function(){
      const result = previousOpenAdminEditor.apply(this, arguments);
      setTimeout(applyFinalUserPatch, 0);
      setTimeout(applyFinalUserPatch, 150);
      setTimeout(applyFinalUserPatch, 500);
      return result;
    };
    patchedOpenAdminEditor.__minchUserFinalPatch = true;
    window.openAdminEditor = patchedOpenAdminEditor;
    try { openAdminEditor = patchedOpenAdminEditor; } catch(e) {}
  }

  const previousOpenBuild = window.openBuild || (typeof openBuild !== 'undefined' ? openBuild : null);
  if (previousOpenBuild && !previousOpenBuild.__minchUserFinalPatch) {
    const patchedOpenBuild = function(){
      const result = previousOpenBuild.apply(this, arguments);
      setTimeout(applyFinalUserPatch, 0);
      setTimeout(applyFinalUserPatch, 500);
      setTimeout(applyFinalUserPatch, 1000);
      return result;
    };
    patchedOpenBuild.__minchUserFinalPatch = true;
    window.openBuild = patchedOpenBuild;
    try { openBuild = patchedOpenBuild; } catch(e) {}
  }

  document.addEventListener('click', function(){ setTimeout(applyFinalUserPatch, 0); }, true);
  ['scroll','resize','orientationchange'].forEach(function(type){
    window.addEventListener(type, function(){
      forceFixedTopButtons();
      setTimeout(forceFixedTopButtons, 120);
    }, { passive:true });
  });

  document.addEventListener('DOMContentLoaded', applyFinalUserPatch);
  setTimeout(applyFinalUserPatch, 0);
  setTimeout(applyFinalUserPatch, 800);
  setInterval(applyFinalUserPatch, 2000);
})();

/* ---------- PATCH FINAL DEMANDE 2 : boutons non fixes accueil + cachés config ---------- */
(function(){
  function byId(id){ return document.getElementById(id); }

  function applyHeaderAndBuildButtonsMode(){
    const music = byId('musicPanel');
    const admin = byId('adminButton');
    const buildBack = document.querySelector('#buildPage .back');
    const isBuildOpen = document.body.classList.contains('build-open');

    if (isBuildOpen) {
      [music, admin].forEach(function(el){
        if (!el) return;
        el.style.setProperty('display', 'none', 'important');
      ...
      if (buildBack) {
        ...
        buildBack.style.setProperty('position', 'fixed', 'important');
        buildBack.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 18px)', 'important');
        buildBack.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 20px)', 'important');
        buildBack.style.setProperty('z-index', '10000050', 'important');
        buildBack.style.setProperty('opacity', '1', 'important');
        buildBack.style.setProperty('visibility', 'visible', 'important');
        buildBack.style.setProperty('pointer-events', 'auto', 'important');
        buildBack.onclick = function(event){
          event.preventDefault();
          event.stopPropagation();
          if (typeof closeBuild === 'function') closeBuild();
    ...
    if (music) {
      music.style.setProperty('display', 'flex', 'important');
      music.style.setProperty('position', 'absolute', 'important');
      music.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 16px)', 'important');
      music.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 16px)', 'important');
      music.style.setProperty('z-index', '5200', 'important');
      music.style.setProperty('opacity', '1', 'important');
      music.style.setProperty('visibility', 'visible', 'important');
      music.style.setProperty('pointer-events', 'auto', 'important');
      music.style.setProperty('transform', 'none', 'important');
    ...
    if (admin) {
      admin.style.setProperty('display', 'flex', 'important');
      admin.style.setProperty('position', 'absolute', 'important');
      admin.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 16px)', 'important');
      admin.style.setProperty('right', 'calc(env(safe-area-inset-right, 0px) + 16px)', 'important');
      admin.style.setProperty('z-index', '5201', 'important');
      admin.style.setProperty('opacity', '1', 'important');
      admin.style.setProperty('visibility', 'visible', 'important');
      admin.style.setProperty('pointer-events', 'auto', 'important');
      admin.style.setProperty('transform', 'none', 'important');
  ...
  const previousOpenBuild = window.openBuild || (typeof openBuild !== 'undefined' ? openBuild : null);
  if (previousOpenBuild && !previousOpenBuild.__minchNoFixedHeaderPatch) {
    const patchedOpenBuild = function(){
      const result = previousOpenBuild.apply(this, arguments);
      setTimeout(applyHeaderAndBuildButtonsMode, 0);
      setTimeout(applyHeaderAndBuildButtonsMode, 120);
      setTimeout(applyHeaderAndBuildButtonsMode, 700);
      setTimeout(applyHeaderAndBuildButtonsMode, 1200);
      return result;
    };
    patchedOpenBuild.__minchNoFixedHeaderPatch = true;
    window.openBuild = patchedOpenBuild;
    try { openBuild = patchedOpenBuild; } catch(e) {}
  }

  const previousCloseBuild = window.closeBuild || (typeof closeBuild !== 'undefined' ? closeBuild : null);
  if (previousCloseBuild && !previousCloseBuild.__minchNoFixedHeaderPatch) {
    const patchedCloseBuild = function(){
      const result = previousCloseBuild.apply(this, arguments);
      setTimeout(applyHeaderAndBuildButtonsMode, 0);
      setTimeout(applyHeaderAndBuildButtonsMode, 500);
      return result;
    };
    patchedCloseBuild.__minchNoFixedHeaderPatch = true;
    window.closeBuild = patchedCloseBuild;
    try { closeBuild = patchedCloseBuild; } catch(e) {}
  }

  ['scroll','resize','orientationchange'].forEach(function(type){
    window.addEventListener(type, function(){
      applyHeaderAndBuildButtonsMode();
      setTimeout(applyHeaderAndBuildButtonsMode, 120);
    }, { passive:true });
  });

  document.addEventListener('DOMContentLoaded', applyHeaderAndBuildButtonsMode);
  setTimeout(applyHeaderAndBuildButtonsMode, 0);
  setTimeout(applyHeaderAndBuildButtonsMode, 500);
  setInterval(applyHeaderAndBuildButtonsMode, 500);
})();

/* ---------- PATCH MINCH V3 : pré-apercu fiable sur chaque upload ---------- */
(function(){
  ...
  function previewFile(input, preview){
    ...
    preview.innerHTML = '<span>Préparation du pré-apercu...</span>';
    ...
    reader.onload = function(){
      ...
      img.onload = function(){
        preview.innerHTML = `
          <img src="${reader.result}" alt="Pré-apercu ${file.name}">
          <div class="minch-preview-info">
            <span class="minch-preview-name">${file.name}</span>
            <span class="minch-preview-meta">${img.naturalWidth} × ${img.naturalHeight} px · ${formatBytes(file.size)}</span>
            <span class="minch-preview-meta">Image prête, compressée à la sauvegarde.</span>
          </div>
        `;
      };
      img.onerror = function(){
        preview.innerHTML = `<span class="minch-preview-name">${file.name}</span><span class="minch-preview-meta">Pré-apercu impossible, fichier gardé.</span>`;
      };
      img.src = reader.result;
    };
  ...
  function enhanceFileInput(input){
    if (!input || input.dataset.minchPreviewV3 === 'true') return;
    input.dataset.minchPreviewV3 = 'true';
    ...
    if (!preview) {
      ...
      preview.className = 'minch-file-preview is-empty';
      preview.textContent = 'Glisse une image ici ou clique pour choisir';
      row.appendChild(preview);
    } else {
      preview.classList.add('is-empty');
      if (!preview.querySelector('img')) preview.textContent = 'Glisse une image ici ou clique pour choisir';
    }
    ...
    input.addEventListener('change', function(){ previewFile(input, preview); });
  ...
  function enhanceAllPreviews(){
    const modal = document.getElementById('adminEditorModal');
    if (!modal || !modal.classList.contains('open')) return;
    modal.querySelectorAll('input[type="file"]').forEach(enhanceFileInput);
  }

  const oldOpenEditor = window.openAdminEditor || (typeof openAdminEditor !== 'undefined' ? openAdminEditor : null);
  if (oldOpenEditor && !oldOpenEditor.__minchPreviewV3) {
    const patched = function(){
      const result = oldOpenEditor.apply(this, arguments);
      setTimeout(enhanceAllPreviews, 50);
      setTimeout(enhanceAllPreviews, 250);
      setTimeout(enhanceAllPreviews, 900);
      return result;
    };
    patched.__minchPreviewV3 = true;
    window.openAdminEditor = patched;
    try { openAdminEditor = patched; } catch(e) {}
  }

  document.addEventListener('change', function(event){
    if (event.target && event.target.matches && event.target.matches('#adminEditorModal input[type="file"]')) {
      setTimeout(enhanceAllPreviews, 0);
    }
  }, true);

  document.addEventListener('DOMContentLoaded', enhanceAllPreviews);
  setTimeout(enhanceAllPreviews, 500);
  window.minchEnhanceUploadPreviews = enhanceAllPreviews;
})();

/* ---------- PATCH MINCH V4 : drag & drop image fiable, sans ouverture nouvel onglet ---------- */
(function(){
  ...
  function nearestFileInput(target){
    if (!target || !target.closest) return null;

    const directInput = target.closest('input[type="file"]');
    if (directInput) return directInput;

    const row = target.closest('.admin-file-row, .minch-drop-row, label');
    if (row) {
      const rowInput = row.querySelector && row.querySelector('input[type="file"]');
      if (rowInput) return rowInput;
    }

    const section = target.closest('.admin-section');
    if (section) {
  ...
  function setFiles(input, files){
    ...
    const selected = imageFiles[0] || files[0];
    ...
    try {
      ...
      console.error('Drag/drop image impossible :', error);
      return false;
    }
  }

  function clearDragState(){
    document.querySelectorAll('.minch-drop-row.drag-over, .admin-file-row.drag-over').forEach(el => el.classList.remove('drag-over'));
  }

  function markDropTarget(target){
    clearDragState();
    const input = nearestFileInput(target);
    if (!input) return;
    const row = input.closest('.admin-file-row') || input.closest('.minch-drop-row') || input.parentElement;
    if (row) row.classList.add('drag-over');
  }

  ['dragenter','dragover'].forEach(type => {
    document.addEventListener(type, function(event){
      if (!editorIsOpen() || !hasDraggedFiles(event)) return;
      event.preventDefault();
      event.stopPropagation();
  ...
  document.addEventListener('dragleave', function(event){
    if (!editorIsOpen() || !hasDraggedFiles(event)) return;
    const modal = document.getElementById('adminEditorModal');
    if (modal && !modal.contains(event.relatedTarget)) clearDragState();
  }, true);
  ...
  document.addEventListener('drop', function(event){
    ...
    const input = nearestFileInput(event.target);
    clearDragState();

    // Important : on bloque toujours l'ouverture de l'image dans le navigateur.
    // Si le drop est bien sur une zone fichier, on injecte l'image dans ce champ.
    if (input) setFiles(input, event.dataTransfer.files);
  }, true);

  // Sécurité globale : empêche le navigateur d'ouvrir une image déposée sur la page.
...
/* ---------- PATCH MINCH V5 : case upload permanente + aperçu intégré ---------- */
(function(){
  function formatBytesV5(bytes){
  ...
  function emptyUploadHTML(){
    return `
      <div class="minch-upload-icon">+</div>
      <div class="minch-upload-title">Dépose ton image ici</div>
      <div class="minch-upload-subtitle">ou clique dans cette case pour choisir une image<br>Pré-apercu affiché directement ici</div>
    `;
  }
  ...
  function showPreview(input, preview){
    ...
    preview.classList.add('minch-upload-card-preview');

    if (!file) {
      preview.classList.add('is-empty');
      preview.innerHTML = emptyUploadHTML();
      return;
    }

    preview.classList.remove('is-empty');
    preview.innerHTML = '<div class="minch-upload-title">Chargement du pré-apercu...</div>';
    ...
    reader.onload = function(){
      ...
      image.onload = function(){
        preview.innerHTML = `
          ...
          <div class="minch-preview-info">
            <span class="minch-preview-name">${file.name}</span>
            <span class="minch-preview-meta">${image.naturalWidth} × ${image.naturalHeight} px · ${formatBytesV5(file.size)}</span>
            <span class="minch-preview-meta">Clique ou redépose une image pour remplacer.</span>
          </div>
        `;
      };
      image.onerror = function(){
        preview.innerHTML = `
          ...
          <div class="minch-preview-info">
            <span class="minch-preview-name">${file.name}</span>
            <span class="minch-preview-meta">Fichier sélectionné, aperçu non disponible.</span>
          </div>
        `;
      };
    ...
    reader.onerror = function(){
      preview.innerHTML = '<div class="minch-upload-title">Image sélectionnée</div>';
    };
    reader.readAsDataURL(file);
  }
  ...
  function setInputFiles(input, files){
    ...
    const file = Array.from(files).find(f => f && f.type && f.type.startsWith('image/')) || files[0];
    try {
      const dt = new DataTransfer();
      dt.items.add(file);
      input.files = dt.files;
      ...
      console.error('Impossible de déposer cette image :', error);
    }
  }

  function upgradeInput(input){
    if (!input || input.dataset.minchUploadCardV5 === 'true') return;
    input.dataset.minchUploadCardV5 = 'true';

    const row = input.closest('.admin-file-row') || input.parentElement;
    if (!row) return;

    row.classList.add('minch-drop-row', 'minch-upload-card-row');
    ...
    preview.className = 'minch-file-preview minch-upload-card-preview is-empty';
    preview.innerHTML = emptyUploadHTML();

    const openChooser = function(event){
      if (event) {
        event.preventDefault();
        event.stopPropagation();
      }
    ...
    row.addEventListener('click', openChooser);
    preview.addEventListener('click', openChooser);
    ...
    row.addEventListener('drop', function(event){
      setInputFiles(input, event.dataTransfer && event.dataTransfer.files);
    });
  }

  function upgradeAllUploadCards(){
    const modal = document.getElementById('adminEditorModal');
    if (!modal || !modal.classList.contains('open')) return;
    modal.querySelectorAll('input[type="file"]').forEach(upgradeInput);
  }

  const previousOpenEditor = window.openAdminEditor || (typeof openAdminEditor !== 'undefined' ? openAdminEditor : null);
  if (previousOpenEditor && !previousOpenEditor.__minchUploadCardV5) {
    const patchedOpenEditor = function(){
      const result = previousOpenEditor.apply(this, arguments);
      setTimeout(upgradeAllUploadCards, 40);
      setTimeout(upgradeAllUploadCards, 180);
      setTimeout(upgradeAllUploadCards, 700);
      return result;
    };
    patchedOpenEditor.__minchUploadCardV5 = true;
    window.openAdminEditor = patchedOpenEditor;
    try { openAdminEditor = patchedOpenEditor; } catch(e) {}
  }
  ...
  document.addEventListener('change', function(event){
    if (event.target && event.target.matches && event.target.matches('#adminEditorModal input[type="file"]')) {
      const row = event.target.closest('.admin-file-row');
      const preview = row && row.querySelector('.minch-file-preview');
      showPreview(event.target, preview);
    }
  }, true);

  document.addEventListener('DOMContentLoaded', upgradeAllUploadCards);
  setTimeout(upgradeAllUploadCards, 300);
  window.minchUpgradeUploadCards = upgradeAllUploadCards;
})();

/* ---------- PATCH MINCH V6 : empêche le flash Admin/Musique sur une config ---------- */
(function(){
  function byId(id){ return document.getElementById(id); }
  ...
  function shouldHideControls(){
    return document.body.classList.contains('build-open') || editorIsOpen();
  }
  ...
  function showOne(el, side){
    if (!el) return;
    el.style.setProperty('display', 'flex', 'important');
    el.style.setProperty('position', 'absolute', 'important');
    el.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 16px)', 'important');
    if (side === 'left') el.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 16px)', 'important');
    if (side === 'right') el.style.setProperty('right', 'calc(env(safe-area-inset-right, 0px) + 16px)', 'important');
  ...
  function keepBackVisible(){
    const back = document.querySelector('#buildPage .back');
    if (!back) return;
    back.style.setProperty('display', 'flex', 'important');
    back.style.setProperty('position', 'fixed', 'important');
    back.style.setProperty('top', 'calc(env(safe-area-inset-top, 0px) + 18px)', 'important');
    back.style.setProperty('left', 'calc(env(safe-area-inset-left, 0px) + 20px)', 'important');
    back.style.setProperty('z-index', '10000060', 'important');
  ...
  function applyNoFlashMode(){
    const music = byId('musicPanel');
    const admin = byId('adminButton');
    const hide = shouldHideControls();
    document.body.classList.toggle('minch-editor-open', editorIsOpen());
  ...
  function applyManyTimes(){
    applyNoFlashMode();
    requestAnimationFrame(applyNoFlashMode);
    setTimeout(applyNoFlashMode, 0);
    setTimeout(applyNoFlashMode, 30);
    setTimeout(applyNoFlashMode, 90);
    setTimeout(applyNoFlashMode, 180);
  }

  const observer = new MutationObserver(applyNoFlashMode);
  observer.observe(document.documentElement, { attributes:true, subtree:true, attributeFilter:['class','style','aria-hidden'] });

  ['pointerdown','mousedown','click','focusin','change','input','dragenter','dragover','drop','keyup'].forEach(function(type){
    document.addEventListener(type, applyManyTimes, true);
  });

  const oldOpenEditor = window.openAdminEditor || (typeof openAdminEditor !== 'undefined' ? openAdminEditor : null);
  if (oldOpenEditor && !oldOpenEditor.__minchNoFlashV6) {
    const patchedOpenEditor = function(){
      applyManyTimes();
      const result = oldOpenEditor.apply(this, arguments);
      applyManyTimes();
    ...
    patchedOpenEditor.__minchNoFlashV6 = true;
    window.openAdminEditor = patchedOpenEditor;
    try { openAdminEditor = patchedOpenEditor; } catch(e) {}
  }

  const oldOpenBuild = window.openBuild || (typeof openBuild !== 'undefined' ? openBuild : null);
  if (oldOpenBuild && !oldOpenBuild.__minchNoFlashV6) {
    const patchedOpenBuild = function(){
      const result = oldOpenBuild.apply(this, arguments);
      applyManyTimes();
      return result;
    };
    patchedOpenBuild.__minchNoFlashV6 = true;
    window.openBuild = patchedOpenBuild;
    try { openBuild = patchedOpenBuild; } catch(e) {}
  }

  const oldCloseEditor = window.closeAdminEditor || (typeof closeAdminEditor !== 'undefined' ? closeAdminEditor : null);
  if (oldCloseEditor && !oldCloseEditor.__minchNoFlashV6) {
    const patchedCloseEditor = function(){
      const result = oldCloseEditor.apply(this, arguments);
      applyManyTimes();
      return result;
    };
    patchedCloseEditor.__minchNoFlashV6 = true;
    window.closeAdminEditor = patchedCloseEditor;
    try { closeAdminEditor = patchedCloseEditor; } catch(e) {}
  }

  document.addEventListener('DOMContentLoaded', applyManyTimes);
  setInterval(applyNoFlashMode, 60);
  applyManyTimes();
})();
