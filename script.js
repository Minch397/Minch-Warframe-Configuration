...
(()=>{
  ...
  function initTop(){let b=document.querySelector('.back-to-top');if(!b){b=document.createElement('button');b.type='button';b.className='back-to-top';b.setAttribute('aria-label','Retour en haut');b.textContent='↑';document.body.appendChild(b);b.onclick=()=>topNow(true)}const update=()=>b.classList.toggle('is-visible',(window.scrollY||document.documentElement.scrollTop)>650);window.addEventListener('scroll',update,{passive:true});update();}

  function init(){initSearch();initTop();decorateRecent();const root=$("buildContent");if(root)new MutationObserver(()=>{setTimeout(()=>{rebuildToc();decorateRecent();if(document.body.classList.contains('build-open'))topNow(false)},30)}).observe(root,{childList:true,subtree:true});new MutationObserver(()=>decorateRecent()).observe(document.body,{childList:true,subtree:true});}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();

/* ---------- MINCH V20 : recommandations aléatoires en bas des configurations ---------- */
(()=>{
  const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]));
  const shuffle=a=>{for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a};

  function getDomItem(rootSelector,type,label){
  ...
  function buildPool(current){
    const pool=[];
    const warframes=(window.warframesData||[]).filter(w=>w && w.name && (!current || w.name!==current.name));
    shuffle(warframes.slice()).slice(0,Math.min(8,warframes.length)).forEach(w=>pool.push({
      type:"warframe", title:w.name, sub:"Configuration Warframe", obj:w,
      image:(typeof getWarframeImage==="function"?getWarframeImage(w.name):"")
    }));
    pool.push(...getDomItem("#companionsGrid","companions","Compagnon"));
    pool.push(...getDomItem("#guidesGrid","guides","Guide & Conseil"));
    pool.push(...getDomItem("#elementsGrid","elements","Élément / Statut"));
    [...document.querySelectorAll("#tierCategoriesGrid [data-tier-category]")].forEach(el=>pool.push({
      type:"tier-category", id:el.dataset.tierCategory||"", title:el.querySelector(".tier-category-title")?.textContent?.trim()||"Tier List", sub:"Tier List",
      image:""
    }));
    pool.push({type:"weekly",title:"Tâches hebdomadaires",sub:"Activités de la semaine",image:""});
    return pool;
  }
  ...
  function chooseRecommendations(current){
    ...
    const chosen=[];
    const take=(arr,n)=>shuffle(arr.slice()).slice(0,n).forEach(x=>{if(!chosen.some(y=>y.type===x.type&&y.id===x.id&&y.title===x.title))chosen.push(x)});
    take(pool.filter(x=>x.type==="warframe"),2);
    take(pool.filter(x=>x.type!=="warframe"),3);
    if(chosen.length<5) take(pool.filter(x=>!chosen.includes(x)),5-chosen.length);
  ...
  function navigateHub(view,id){
    const go=()=>{
      ...
      setTimeout(()=>{
        if(view==="tiers" && id){
          document.querySelector(`#tierCategoriesGrid [data-tier-category="${CSS.escape(String(id))}"]`)?.click();
          return;
        }
        if(id){
          const root=view==="weekly"?document.getElementById("weeklyTasks"):document.getElementById(`${view}Grid`);
          root?.querySelector(`[data-id="${CSS.escape(String(id))}"]`)?.scrollIntoView({behavior:"smooth",block:"center"});
        }
      },220);
    };
    if(document.body.classList.contains("build-open") && typeof closeBuild==="function"){
      closeBuild();
      setTimeout(go,520);
    }else go();
  }
  ...
  function openRecommendation(item){
    if(item.type==="warframe" && item.obj){
      if(typeof openBuild==="function") openBuild(item.obj);
      return;
    }
  ...
  function renderRecommendations(){
    ...
    if(!current) return;
    const items=chooseRecommendations(current);
    if(!items.length) return;
    const section=document.createElement("section");
    section.className="build-recommendations";
    section.innerHTML=`
      <div class="build-recommendations-head"><span></span><h2>Recommandations</h2><span></span></div>
      <div class="build-recommendations-grid">
        ${items.map((x,i)=>`<button class="build-recommendation-card" type="button" data-rec-index="${i}">
          <span class="build-recommendation-media" ${x.image?`style="background-image:linear-gradient(rgba(4,10,20,.18),rgba(4,10,20,.38)),url('${esc(x.image)}')"`:""}>${x.image?"":`<span class="build-recommendation-fallback">${x.type==="weekly"?"✓":x.type==="tier-category"?"★":x.type==="companions"?"🐾":x.type==="guides"?"✦":x.type==="elements"?"◈":"WF"}</span>`}</span>
          <span class="build-recommendation-copy"><small>${esc(x.sub)}</small><strong>${esc(x.title)}</strong></span><span class="build-recommendation-arrow">→</span>
        </button>`).join("")}
      </div>`;
    root.appendChild(section);
    section.querySelectorAll("[data-rec-index]").forEach(btn=>btn.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();openRecommendation(items[Number(btn.dataset.recIndex)])}));
  }

  function init(){
    const root=document.getElementById("buildContent");
    if(!root)return;
    new MutationObserver(()=>setTimeout(renderRecommendations,40)).observe(root,{childList:true,subtree:false});
    document.addEventListener("click",e=>{if(e.target.closest?.(".card"))setTimeout(renderRecommendations,700)},true);
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init,{once:true});else init();
})();
