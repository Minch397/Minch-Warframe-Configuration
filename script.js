...
/* ---------- PATCH SAUVEGARDE FIREBASE STABLE : multi-configs + images ---------- */
/* Ce bloc ne change pas l'esthétique : il sécurise seulement la sauvegarde en ligne. */
const MINCH_STABLE_FIREBASE_CONFIG = {
...
const MINCH_STABLE_FIRESTORE_COLLECTION = "warframe_site";
const MINCH_STABLE_FIRESTORE_DOCUMENT = "main";
let firebaseReady = false;
let firebaseDb = null;
let firebaseStorage = null;

function minchStableSlug(value){
  if (typeof slugifyText === "function") return slugifyText(value || "image");
  return String(value || "image").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "image";
}

function initMinchStableFirebase(){
  if (firebaseReady && firebaseDb && firebaseStorage) return true;
  if (!window.firebase || !MINCH_STABLE_FIREBASE_CONFIG.apiKey) return false;

  try {
    if (!firebase.apps || !firebase.apps.length) {
      firebase.initializeApp(MINCH_STABLE_FIREBASE_CONFIG);
    }
...
async function minchStableCompressOrRead(file){
  if (typeof minchCompressImageFile === "function") {
    const compressed = await minchCompressImageFile(file);
...
async function uploadImageInput(inputId, folderName){
  ...
  const file = input.files[0];
  const dataUrl = await minchStableCompressOrRead(file);
  if (!dataUrl) return null;

  if (!initMinchStableFirebase() || !firebaseStorage) {
    return dataUrl;
  }
  ...
  try {
    ...
    const currentNameInput = document.getElementById("editWarframeName");
    const warframeSlug = minchStableSlug(currentNameInput ? currentNameInput.value : "configuration");
    const cleanFolder = minchStableSlug(folderName || "images");
    const cleanFile = minchStableSlug(file.name.replace(/\.[^.]+$/, ""));
    const fileName = `${Date.now()}_${cleanFile}.${ext}`;
    const ref = firebaseStorage.ref().child(`warframe-images/${warframeSlug}/${cleanFolder}/${fileName}`);

    await ref.put(blob, {
      contentType: blob.type || "image/webp",
      customMetadata: { originalName: file.name, inputId }
    });

    return await ref.getDownloadURL();
  } catch(error) {
    console.error("Upload Firebase Storage impossible, fallback local :", error);
    return dataUrl;
  }
}
window.uploadImageInput = uploadImageInput;

async function minchStableLoadOnlineData(){
  if (!initMinchStableFirebase() || !firebaseDb) return false;

  try {
    const doc = await firebaseDb
      .collection(MINCH_STABLE_FIRESTORE_COLLECTION)
      .doc(MINCH_STABLE_FIRESTORE_DOCUMENT)
      .get();

    if (!doc.exists) return false;
    const data = doc.data() || {};
    if (!Array.isArray(data.warframesData)) return false;

    warframesData = data.warframesData;
    localStorage.setItem(ADMIN_DATA_KEY, JSON.stringify(warframesData));
    renderGrid();

    if (window.currentOpenWarframe && document.body.classList.contains("build-open")) {
      const fresh = warframesData.find(w => w.name === window.currentOpenWarframe.name);
      if (fresh) {
        window.currentOpenWarframe = fresh;
        fillBuildContent(fresh);
      }
    }

    return true;
  } catch(error) {
    console.error("Chargement Firebase impossible :", error);
    return false;
  }
}

async function minchStableSaveOnlineData(){
  if (!initMinchStableFirebase() || !firebaseDb) return false;
...
saveAdminData = function(){
  ...
  minchStableSaveOnlineData().then((savedOnline) => {
    const status = document.getElementById("adminSaveStatus");
    if (!status) return;
    status.textContent = savedOnline
      ? "Sauvegardé en ligne sur Firebase."
      : "Sauvegardé dans ce navigateur. Firebase indisponible.";
  });
};
window.saveAdminData = saveAdminData;

(function minchStableStartupSync(){
  initMinchStableFirebase();
  minchStableLoadOnlineData();
})();
