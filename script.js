...
/* ---------- FIX FINAL LOGIN GOOGLE AUTOFILL + DOUBLE CLIC ADMIN ---------- */
function finalSetPasswordEyeState() {
  const passInput = document.getElementById("adminPass");
  const eye = document.getElementById("adminPasswordToggle");
...
function finalToggleAdminPassword(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  ...
  finalSetPasswordEyeState();
}

function finalTryAdminLogin(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
    if (event.stopImmediatePropagation) event.stopImmediatePropagation();
  ...
  const modal = document.getElementById("adminLoginModal");
  const connectedView = Boolean(modal && modal.classList.contains("admin-connected-view"));

  if (window.isAdminMode && connectedView) {
    logoutAdmin();
    closeAdminLogin();
    return;
  }
  ...
  const user = String(userInput?.value || "").trim().toUpperCase();
  ...
  if (user === ADMIN_USERNAME && pass === ADMIN_PASSWORD) {
    ...
    renderGrid();
    if (window.currentOpenWarframe && document.body.classList.contains("build-open")) {
      fillBuildContent(window.currentOpenWarframe);
    }
  } else if (error) {
    error.textContent = "Identifiant ou mot de passe incorrect.";
  }
...
function finalPrepareAdminLoginModal() {
  const modal = document.getElementById("adminLoginModal");
  if (!modal) return;
  ...
  if (passWrap) passWrap.style.display = window.isAdminMode ? "none" : "block";
  if (userInput) userInput.style.display = window.isAdminMode ? "none" : "block";
  if (passInput && !window.isAdminMode) passInput.type = "password";
  finalSetPasswordEyeState();

  if (eye) {
    const cleanEye = eye.cloneNode(true);
    eye.replaceWith(cleanEye);
    cleanEye.addEventListener("click", finalToggleAdminPassword, true);
  }

  if (submit) {
    const cleanSubmit = submit.cloneNode(true);
    submit.replaceWith(cleanSubmit);
    cleanSubmit.addEventListener("click", finalTryAdminLogin, true);
  }
}

const minchOpenAdminLoginFixed = openAdminLogin;
openAdminLogin = function() {
  minchOpenAdminLoginFixed();
  showAdminConnectedState();
  finalPrepareAdminLoginModal();
  setTimeout(finalPrepareAdminLoginModal, 50);
};
...
["adminUser", "adminPass"].forEach((id) => {
  ...
  input.addEventListener("keydown", (event) => {
    if (event.key === "Enter") finalTryAdminLogin(event);
  }, true);
});

window.tryAdminLogin = finalTryAdminLogin;
window.openAdminLogin = openAdminLogin;
...
/* ---------- PATCH DÉFINITIF MINCH : LOGIN ADMIN + AUTOFILL ---------- */
function minchNormalizeLoginValue(value) {
  return String(value || "")
    .replace(/[\u200B-\u200D\uFEFF]/g, "")
    .trim();
}

function minchUpdateEyeIcon() {
  const passInput = document.getElementById("adminPass");
  const eye = document.getElementById("adminPasswordToggle");
  if (!passInput || !eye) return;
  const visible = passInput.type === "text";
...
function minchForceDarkAdminInputs() {
  ["adminUser", "adminPass"].forEach((id) => {
    const input = document.getElementById(id);
    if (!input) return;
    input.classList.add("minch-dark-input-force");
...
function minchTogglePassword(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
    if (event.stopImmediatePropagation) event.stopImmediatePropagation();
  ...
  passInput.type = passInput.type === "password" ? "text" : "password";
  minchUpdateEyeIcon();
  minchForceDarkAdminInputs();
}

function minchDoAdminLogin(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
    if (event.stopImmediatePropagation) event.stopImmediatePropagation();
  ...
  if (window.isAdminMode && modal && modal.classList.contains("admin-connected-view")) {
    logoutAdmin();
    closeAdminLogin();
    return false;
  }
  ...
  const userInput = document.getElementById("adminUser");
  const passInput = document.getElementById("adminPass");
  const user = minchNormalizeLoginValue(userInput ? userInput.value : "").toUpperCase();
  const pass = minchNormalizeLoginValue(passInput ? passInput.value : "");
...
function minchPrepareLoginModalFinal() {
  const modal = document.getElementById("adminLoginModal");
  if (!modal) return;
  ...
  if (submit) submit.onclick = minchDoAdminLogin;
  if (eye) eye.onclick = minchTogglePassword;
}

const minchPreviousOpenAdminLogin = openAdminLogin;
openAdminLogin = function() {
  minchPreviousOpenAdminLogin();
  showAdminConnectedState();
  minchPrepareLoginModalFinal();
  setTimeout(minchPrepareLoginModalFinal, 60);
  setTimeout(minchForceDarkAdminInputs, 250);
};

tryAdminLogin = minchDoAdminLogin;
window.tryAdminLogin = minchDoAdminLogin;
window.openAdminLogin = openAdminLogin;

// Capture au niveau document : même si un ancien listener existe, celui-ci passe en premier.
document.addEventListener("click", (event) => {
  const submit = event.target.closest && event.target.closest("#adminLoginSubmit");
  const eye = event.target.closest && event.target.closest("#adminPasswordToggle");
  if (submit) minchDoAdminLogin(event);
  if (eye) minchTogglePassword(event);
}, true);

document.addEventListener("keydown", (event) => {
  if ((event.target?.id === "adminUser" || event.target?.id === "adminPass") && event.key === "Enter") {
    minchDoAdminLogin(event);
  }
}, true);

["input", "change", "animationstart", "focus", "blur"].forEach((eventName) => {
  document.addEventListener(eventName, (event) => {
    if (event.target && (event.target.id === "adminUser" || event.target.id === "adminPass")) {
      setTimeout(minchForceDarkAdminInputs, 0);
      setTimeout(minchForceDarkAdminInputs, 80);
    }
  }, true);
});
...
/* ---------- PATCH MINCH SURFACE PERSONNALISATION COMPLÈTE ---------- */
function minchSectionDefaults(build) {
  if (!build.sections) build.sections = {};
  const defaults = {
    fragments: "Fragments d'Archonte",
    specialWeapons: "Armes exaltées / propres à la Warframe",
  ...
  Object.keys(defaults).forEach((key) => {
    if (!build.sections[key]) build.sections[key] = {};
    if (typeof build.sections[key].enabled !== "boolean") build.sections[key].enabled = true;
    if (typeof build.sections[key].textEnabled !== "boolean") build.sections[key].textEnabled = false;
    if (typeof build.sections[key].text !== "string") build.sections[key].text = "";
    if (typeof build.sections[key].title !== "string") build.sections[key].title = defaults[key];
  });

  if (!build.specialWeapons) {
    build.specialWeapons = { enabled: false, title: "Armes exaltées / propres à la Warframe", items: [] };
  }
  if (!Array.isArray(build.specialWeapons.items)) build.specialWeapons.items = [];
  if (typeof build.specialWeapons.enabled !== "boolean") build.specialWeapons.enabled = false;
  if (!build.specialWeapons.title) build.specialWeapons.title = "Armes exaltées / propres à la Warframe";

  while (build.specialWeapons.items.length < 3) {
    const index = build.specialWeapons.items.length;
    build.specialWeapons.items.push({
      enabled: index === 0,
      name: `Arme spéciale ${index + 1}`,
      image: "",
      buildImage: ""
    });
  }
...
function minchGetSection(build, key) {
  return minchSectionDefaults(build)[key];
}

function minchShouldShowSection(build, key) {
  const section = minchGetSection(build, key);
  return section.enabled !== false;
}

function minchFallbackHTML(build, key, defaultTitle) {
  const section = minchGetSection(build, key);
  if (section.enabled !== false || !section.textEnabled || !section.text) return "";
  return `
    <div class="weapon-section minch-fallback-section">
      <h2>${escapeHtml(section.title || defaultTitle)}</h2>
      <div class="weapon-text-note">${escapeHtml(section.text)}</div>
    </div>
  `;
}

function minchRenderFragmentsBlock(selectedBuild) {
  const section = minchGetSection(selectedBuild, "fragments");
  if (section.enabled === false) return minchFallbackHTML(selectedBuild, "fragments", "Fragments d'Archonte");

  const shardsHTML = buildShardLine(selectedBuild.fragments || []);
  if (!shardsHTML) return "";
  return `
    <h2>${escapeHtml(section.title || "Fragments d'Archonte")}</h2>
    <div class="shards-container">${shardsHTML}</div>
  `;
}

renderSpecialWeaponsBlock = function(selectedBuild, warframeName) {
  minchSectionDefaults(selectedBuild);
  const section = minchGetSection(selectedBuild, "specialWeapons");
  const specialWeapons = selectedBuild.specialWeapons || {};
  const items = Array.isArray(specialWeapons.items) ? specialWeapons.items : [];
  const visibleItems = items.slice(0, 3).filter(item => item && item.enabled !== false);

  if (section.enabled === false || specialWeapons.enabled === false || visibleItems.length === 0) {
    return minchFallbackHTML(selectedBuild, "specialWeapons", "Armes exaltées / propres à la Warframe");
  }

  const rows = visibleItems.map((weapon, index) => {
    const weaponName = weapon.name || `Arme spéciale ${index + 1}`;
    const weaponImage = weapon.image || getSpecialWeaponImage(warframeName, index);
    const weaponBuild = weapon.buildImage || getSpecialWeaponBuildImage(warframeName, index);
    return `
      <div class="weapon-row">
        <div class="weapon-left weapon-mini">
          <h3>${escapeHtml(weaponName)}</h3>
          <img src="${weaponImage}" alt="${escapeHtml(weaponName)}">
        </div>
        <div class="weapon-right weapon-build-big">
          <h3>Configuration</h3>
          <img src="${weaponBuild}" alt="Configuration ${escapeHtml(weaponName)}">
        </div>
      </div>
    `;
  }).join("");
  ...
  return `
    <div class="weapon-section special-weapon-section">
      <h2>${escapeHtml(specialWeapons.title || section.title || "Armes exaltées / propres à la Warframe")}</h2>
      ${rows}
    </div>
  `;
};

renderWeaponsBlock = function(selectedBuild, warframeName) {
  minchSectionDefaults(selectedBuild);
  const slots = [
    ["principale", "Principale"],
    ["secondaire", "Secondaire"],
  ...
  return slots.map(([slotKey, title]) => {
    const section = minchGetSection(selectedBuild, slotKey);
    if (section.enabled === false) return minchFallbackHTML(selectedBuild, slotKey, title);

    const weaponData = getWeaponData(selectedBuild, slotKey, warframeName);
    const incarnonHTML = renderIncarnonBlock(selectedBuild, warframeName, slotKey);
    return renderWeaponSection(section.title || title, weaponData, incarnonHTML);
  }).join("");
};

renderCompanionBlock = function(selectedBuild, warframeName) {
  minchSectionDefaults(selectedBuild);
  const section = minchGetSection(selectedBuild, "companion");
  if (section.enabled === false || selectedBuild.companionMode === "text") {
    const text = section.text || selectedBuild.companionText || "Vous pouvez jouer n'importe quel compagnon";
    if (section.textEnabled || selectedBuild.companionMode === "text") {
      return `
        <div class="weapon-section">
          <h2>${escapeHtml(section.title || "Compagnon")}</h2>
          <div class="weapon-text-note">${escapeHtml(text)}</div>
        </div>
      `;
    }
    return "";
  ...
  return `
    <div class="weapon-section companion-section">
      ...
      <div class="companion-row">
        <div class="companion-left companion-mini">
          <h3>${escapeHtml(companionData.name)}</h3>
          <img src="${companionData.image}" alt="${escapeHtml(companionData.name)}">
        </div>
        <div class="companion-right companion-build-big">
          <h3>Configuration du compagnon</h3>
          <img src="${companionData.buildImage}" alt="Configuration ${escapeHtml(companionData.name)}">
        </div>
      </div>

      <div class="companion-row">
        <div class="companion-left companion-mini">
          <h3>${escapeHtml(companionData.weaponName)}</h3>
          <img src="${companionData.weaponImage}" alt="${escapeHtml(companionData.weaponName)}">
        </div>
        <div class="companion-right companion-build-big">
          <h3>Configuration de l'arme du compagnon</h3>
          <img src="${companionData.weaponBuildImage}" alt="Configuration ${escapeHtml(companionData.weaponName)}">
        </div>
      </div>

      <div class="companion-row companion-skin-row">
        <div class="companion-left companion-mini">
          <h3>${escapeHtml(companionData.skinName)}</h3>
          <img src="${companionData.skinImage}" alt="${escapeHtml(companionData.skinName)}">
        </div>
      </div>
    </div>
  `;
...
fillBuildContent = function(w) {
  ...
  const fashionImgSrc = selectedBuild.fashionImage || getFashionImage(w.name);
  const buildImgSrc = selectedBuild.buildImage || getBuildImage(w.name);
  const specialWeaponsHTML = renderSpecialWeaponsBlock(selectedBuild, w.name);
  const fragmentsHTML = minchRenderFragmentsBlock(selectedBuild);
  const weaponsHTML = renderWeaponsBlock(selectedBuild, w.name);
  const companionHTML = renderCompanionBlock(selectedBuild, w.name);
  const configInfoHTML = selectedBuild.infoText
    ? `
  ...
  const videoHTML = selectedBuild.video
    ? `
      <h2>Vidéo explicative</h2>
      <div class="video">
        <iframe src="${selectedBuild.video}" title="Vidéo explicative ${escapeHtml(w.name)}" allowfullscreen></iframe>
      </div>
    `
    : "";
  ...
  document.getElementById("buildContent").innerHTML = `
    <h2>Skin</h2>
    <img src="${fashionImgSrc}" alt="Skin ${escapeHtml(w.name)}">

    <h2>${escapeHtml(selectedBuild.name || `${w.name} Configuration`)}</h2>
    <img src="${buildImgSrc}" alt="Configuration ${escapeHtml(w.name)}">
  ...
  if (window.isAdminMode) {
    ...
    `;
    document.getElementById("buildContent").prepend(adminBar);
    if (typeof attachAdminFloatingButtons === "function") attachAdminFloatingButtons();
  }
};

function minchCheckbox(id, label, checked) {
  return `
    <label class="minch-checkline">
...
function minchTextValue(value) {
  return escapeHtml(value || "");
}

function minchSectionEditor(build, key, title) {
  const section = minchGetSection(build, key);
  return `
    <div class="minch-section-options">
      <div class="admin-grid-form">
        <label>Titre de la section<input id="sectionTitle_${key}" class="admin-input" value="${minchTextValue(section.title || title)}"></label>
        <div>
          ${minchCheckbox(`sectionEnabled_${key}`, "Afficher cette section", section.enabled !== false)}
          ${minchCheckbox(`sectionTextEnabled_${key}`, "Afficher un texte si la section est masquée", Boolean(section.textEnabled))}
        </div>
      </div>
      <label>Texte de remplacement<textarea id="sectionText_${key}" class="admin-textarea" placeholder="Texte affiché si la section est masquée...">${minchTextValue(section.text || "")}</textarea></label>
    </div>
  `;
}

function minchSpecialWeaponsEditor(build) {
  minchSectionDefaults(build);
  const special = build.specialWeapons;
  const items = special.items.slice(0, 3);

  return `
    <div class="admin-section">
      <h3>Armes exaltées / propres à la Warframe</h3>
      ${minchSectionEditor(build, "specialWeapons", "Armes exaltées / propres à la Warframe")}
      ${minchCheckbox("editSpecialEnabled", "Activer le bloc armes exaltées", special.enabled !== false)}
      <label>Nom global du bloc<input id="editSpecialTitle" class="admin-input" value="${minchTextValue(special.title || "Armes exaltées / propres à la Warframe")}"></label>
      <p class="admin-small">Tu peux afficher jusqu'à 3 armes. Chaque arme a son image à gauche et sa configuration à droite.</p>
      ${items.map((item, index) => `
        <div class="admin-section minch-mini-editor">
          <h4>Arme exaltée ${index + 1}</h4>
          ${minchCheckbox(`editSpecialItemEnabled_${index}`, `Afficher l'arme ${index + 1}`, item.enabled !== false)}
          <label>Nom<input id="editSpecialName_${index}" class="admin-input" value="${minchTextValue(item.name || `Arme spéciale ${index + 1}`)}"></label>
          <div class="admin-grid-form">
            <label>Image arme<div class="admin-file-row"><input id="editSpecialImage_${index}" type="file" accept="image/*"></div></label>
            <label>Image configuration<div class="admin-file-row"><input id="editSpecialBuild_${index}" type="file" accept="image/*"></div></label>
          </div>
          <p class="admin-small">Si tu ne mets pas d'image, le site garde l'image déjà existante ou le chemin automatique.</p>
        </div>
      `).join("")}
    </div>
  `;
...
openAdminEditor = function(warframe) {
  ...
  content.innerHTML = `
    ...
    <div class="admin-section">
      <h3>Infos principales</h3>
      <div class="admin-grid-form">
        <label>Nom Warframe<input id="editWarframeName" class="admin-input" value="${minchTextValue(editingWarframe.name)}"></label>
        <label>Titre configuration<input id="editBuildName" class="admin-input" value="${minchTextValue(build.name || editingWarframe.name + " Configuration")}"></label>
      </div>
      <label>Lien YouTube<input id="editVideo" class="admin-input" value="${minchTextValue(build.video || "")}" placeholder="https://www.youtube.com/watch?v=..."></label>
      <label>Informations sur la configuration<textarea id="editInfoText" class="admin-textarea" placeholder="Explique ici comment jouer la config, pourquoi utiliser ce build, les astuces, etc.">${minchTextValue(build.infoText || "")}</textarea></label>
      <div class="admin-grid-form">
        <label>Image Warframe / carte<div class="admin-file-row"><input id="editCardImage" type="file" accept="image/*"></div></label>
        <label>Image Skin<div class="admin-file-row"><input id="editFashionImage" type="file" accept="image/*"></div></label>
        <label>Image Build Warframe<div class="admin-file-row"><input id="editBuildImage" type="file" accept="image/*"></div></label>
    ...
    <div class="admin-section">
      <h3>Fragments d'Archonte</h3>
      ${minchSectionEditor(build, "fragments", "Fragments d'Archonte")}
      <p class="admin-small">Une ligne par fragment : clé|texte. Exemples : tau_ambre, tau_ecarlate, tau_violet, ambre, ecarlate.</p>
      <textarea id="editFragments" class="admin-textarea">${minchTextValue(fragmentsToText(build.fragments))}</textarea>
    </div>

    ${minchSpecialWeaponsEditor(build)}

    <div class="admin-section">
      <h3>Armes</h3>
      <p class="admin-small">Chaque arme peut être affichée, masquée, ou remplacée par un texte.</p>
      ${["principale", "secondaire", "melee"].map(slot => {
        const title = slot === "melee" ? "Mêlée" : slot.charAt(0).toUpperCase() + slot.slice(1);
        return `
          <div class="admin-section minch-mini-editor">
            <h4>${title}</h4>
            ${minchSectionEditor(build, slot, title)}
            <label>Nom<input id="editWeaponName_${slot}" class="admin-input" value="${minchTextValue(weapons[slot]?.name || "")}"></label>
            <div class="admin-grid-form">
              <label>Image arme<div class="admin-file-row"><input id="editWeaponImage_${slot}" type="file" accept="image/*"></div></label>
              <label>Image build arme<div class="admin-file-row"><input id="editWeaponBuild_${slot}" type="file" accept="image/*"></div></label>
            </div>
    ...
    <div class="admin-section">
      <h3>Compagnon</h3>
      ${minchSectionEditor(build, "companion", "Compagnon")}
      <label>Mode compagnon
        <select id="editCompanionMode" class="admin-select">
          <option value="full" ${build.companionMode !== "text" ? "selected" : ""}>Images + builds</option>
          <option value="text" ${build.companionMode === "text" ? "selected" : ""}>Texte seulement</option>
      ...
      <label>Texte compagnon<input id="editCompanionText" class="admin-input" value="${minchTextValue(build.companionText || "Vous pouvez jouer n'importe quel compagnon")}"></label>
      <div class="admin-grid-form">
        <label>Nom compagnon<input id="editCompanionName" class="admin-input" value="${minchTextValue(companion.name || "")}"></label>
        <label>Nom arme compagnon<input id="editCompanionWeaponName" class="admin-input" value="${minchTextValue(companion.weaponName || "")}"></label>
        <label>Nom skin<input id="editCompanionSkinName" class="admin-input" value="${minchTextValue(companion.skinName || "")}"></label>
      </div>
      <div class="admin-grid-form">
        <label>Image compagnon<div class="admin-file-row"><input id="editCompanionImage" type="file" accept="image/*"></div></label>
        <label>Build compagnon<div class="admin-file-row"><input id="editCompanionBuildImage" type="file" accept="image/*"></div></label>
  ...
  document.getElementById("saveAdminEdit").onclick = () => minchSaveAdminEditor(editingWarframe, isNew, warframe?.name);
...
function minchReadSectionFromEditor(build, key) {
  minchSectionDefaults(build);
  const section = build.sections[key];
  const enabled = document.getElementById(`sectionEnabled_${key}`);
  const textEnabled = document.getElementById(`sectionTextEnabled_${key}`);
  const title = document.getElementById(`sectionTitle_${key}`);
  const text = document.getElementById(`sectionText_${key}`);
  section.enabled = enabled ? enabled.checked : true;
  section.textEnabled = textEnabled ? textEnabled.checked : false;
  section.title = title ? title.value.trim() : section.title;
  section.text = text ? text.value.trim() : "";
}

async function minchSaveAdminEditor(editingWarframe, isNew, originalName) {
  const saveButton = document.getElementById("saveAdminEdit");
  const status = document.getElementById("adminSaveStatus");
  if (saveButton) saveButton.disabled = true;
  if (status) status.textContent = "Sauvegarde en cours...";
  ...
  try {
    ...
    ["fragments", "specialWeapons", "principale", "secondaire", "melee", "companion"].forEach(key => minchReadSectionFromEditor(build, key));

    build.specialWeapons.enabled = document.getElementById("editSpecialEnabled")?.checked ?? true;
    build.specialWeapons.title = document.getElementById("editSpecialTitle")?.value.trim() || "Armes exaltées / propres à la Warframe";
    build.specialWeapons.items = build.specialWeapons.items.slice(0, 3).map((item, index) => ({
      ...item,
      enabled: document.getElementById(`editSpecialItemEnabled_${index}`)?.checked ?? false,
      name: document.getElementById(`editSpecialName_${index}`)?.value.trim() || `Arme spéciale ${index + 1}`
    }));

    build.weaponMode = "full";
    if (!build.weapons) build.weapons = {};
    ["principale", "secondaire", "melee"].forEach(slot => {
      if (!build.weapons[slot]) build.weapons[slot] = {};
      build.weapons[slot].name = document.getElementById(`editWeaponName_${slot}`).value.trim() || "Nom d'arme";
    ...
    const uploadJobs = [
      ...
      ...[0, 1, 2].flatMap(index => [
        [`editSpecialImage_${index}`, `special-weapons/${index + 1}`, data => build.specialWeapons.items[index].image = data],
        [`editSpecialBuild_${index}`, `special-weapon-builds/${index + 1}`, data => build.specialWeapons.items[index].buildImage = data]
      ])
    ];
...
/* ---------- PATCH FINAL RÉGRESSION PERFORMANCE : recherche + admin + flèches + images ---------- */
(function(){
  const PRELOADED = new Set();

  function safeCall(fn, ...args){
    try { return typeof fn === 'function' ? fn(...args) : undefined; } catch(e){ console.error(e); }
  }

  function preloadImage(src, priority=false){
    if (!src || typeof src !== 'string' || src.startsWith('data:') || PRELOADED.has(src)) return;
    PRELOADED.add(src);
  ...
  function optimizeImage(img, priority=false){
  ...
  function optimizeImages(root=document, priorityCount=0){
    Array.from(root.querySelectorAll('img')).forEach((img, i) => optimizeImage(img, i < priorityCount));
  }

  function preloadHomeImages(list){
    (list || window.warframesData || warframesData || []).slice(0, 8).forEach((w, i) => {
      if (!w) return;
      preloadImage(w.cardImage || getWarframeImage(w.name), i < 4);
    });
  }

  function preloadBuildImages(w){
    if (!w || !w.builds || !w.builds[0]) return;
    const b = w.builds[0];
    preloadImage(w.cardImage || getWarframeImage(w.name), true);
    preloadImage(b.fashionImage || getFashionImage(w.name), true);
    preloadImage(b.buildImage || getBuildImage(w.name), true);
    if (b.specialWeapons && Array.isArray(b.specialWeapons.items)) {
      b.specialWeapons.items.slice(0,3).forEach((weapon, index) => {
        preloadImage(weapon.image || getSpecialWeaponImage(w.name, index));
        preloadImage(weapon.buildImage || getSpecialWeaponBuildImage(w.name, index));
      });
    }
  }
  ...
  function removeFakeAdminButtons(){
    document.querySelectorAll('body > button').forEach((btn) => {
      if (btn.id !== 'adminButton' && btn.textContent.trim().toLowerCase() === 'admin') btn.remove();
    });
  }

  function forceDarkLoginInputs(){
    ['adminUser','adminPass'].forEach((id) => {
      const input = document.getElementById(id);
      if (!input) return;
      input.style.backgroundColor = '#0b1222';
  ...
  function togglePassword(event){
    ...
    syncEye();
    forceDarkLoginInputs();
    return false;
  }

  function setAdminLoginView(){
    const modal = document.getElementById('adminLoginModal');
    const title = document.querySelector('#adminLoginModal .admin-login-card h2');
    const hint = document.querySelector('#adminLoginModal .admin-hint');
    const user = document.getElementById('adminUser');
    ...
    const submit = document.getElementById('adminLoginSubmit');
    const error = document.getElementById('adminLoginError');
    if (!modal || !title || !hint || !user || !passWrap || !submit) return;
  ...
  function openAdminPanel(event){
    ...
    modal.setAttribute('aria-hidden','false');
    setAdminLoginView();
    setTimeout(forceDarkLoginInputs, 30);
    return false;
  }
  ...
  function doAdminLogin(event){
    ...
    const user = String(document.getElementById('adminUser')?.value || '').trim().toUpperCase();
  ...
  function bindControls(){
    ...
    const buildBack = document.querySelector('#buildPage .back');
    if (buildBack) buildBack.onclick = (event) => {
      event.preventDefault();
      event.stopPropagation();
      closeBuild();
      return false;
    ...
    const loginBack = document.querySelector('#adminLoginModal .admin-back');
    if (loginBack) loginBack.onclick = (event) => {
      event.preventDefault();
      event.stopPropagation();
      closeAdminLogin();
      return false;
    ...
    const editorBack = document.querySelector('#adminEditorModal .admin-back');
    if (editorBack) editorBack.onclick = (event) => {
      event.preventDefault();
      event.stopPropagation();
      closeAdminEditor();
      return false;
    ...
    const submit = document.getElementById('adminLoginSubmit');
    if (submit) submit.onclick = doAdminLogin;

    const eye = document.getElementById('adminPasswordToggle');
    if (eye) eye.onclick = togglePassword;
  }

  function fixedCloseBuild(){
    if (buildPage.style.display === 'none' || !document.body.classList.contains('build-open')) return;
    pageTransition.classList.add('active');
    setTimeout(() => {
      buildPage.style.display = 'none';
      document.body.classList.remove('build-open');
      setTimeout(() => pageTransition.classList.remove('active'), 120);
    }, 420);
  }

  const previousRenderWarframes = renderWarframes;
  renderWarframes = function(list){
    preloadHomeImages(list);
    previousRenderWarframes(list);
    optimizeImages(grid, 4);
    bindControls();
  };

  filterWarframes = function(){
    const input = String(document.getElementById('search')?.value || '').toLowerCase().trim();
    const filtered = (warframesData || []).filter((w) => String(w.name || '').toLowerCase().includes(input));
    renderWarframes(filtered);
  };

  const previousFillBuildContent = fillBuildContent;
  fillBuildContent = function(w){
    preloadBuildImages(w);
    previousFillBuildContent(w);
    const buildContent = document.getElementById('buildContent');
    if (buildContent) optimizeImages(buildContent, 2);
    bindControls();
  };
  ...
  openBuild = function(w){
    ...
    preloadBuildImages(w);
    previousOpenBuild(w);
    setTimeout(() => { bindControls(); optimizeImages(document.getElementById('buildContent') || document, 2); }, 650);
  };

  closeBuild = fixedCloseBuild;

  document.addEventListener('click', function(event){
    const actionBtn = event.target.closest && event.target.closest('[data-admin-build-action]');
    if (actionBtn) {
      event.preventDefault();
      event.stopPropagation();
      if (event.stopImmediatePropagation) event.stopImmediatePropagation();
      const action = actionBtn.getAttribute('data-admin-build-action');
      if (action === 'edit' && window.currentOpenWarframe) openAdminEditor(window.currentOpenWarframe);
      if (action === 'duplicate') duplicateCurrentWarframe();
      if (action === 'delete') deleteCurrentWarframe();
      return false;
    }

    if (event.target.closest && event.target.closest('#adminButton')) return openAdminPanel(event);
    if (event.target.closest && event.target.closest('#adminLoginSubmit')) return doAdminLogin(event);
    if (event.target.closest && event.target.closest('#adminPasswordToggle')) return togglePassword(event);
    if (event.target.closest && event.target.closest('#buildPage .back')) {
      event.preventDefault(); event.stopPropagation(); closeBuild(); return false;
    }
  }, true);
  ...
  ['adminUser','adminPass'].forEach((id) => {
    ...
    if (input) {
      input.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') doAdminLogin(event);
      }, true);
    }
  });

  const search = document.getElementById('search');
  if (search) search.addEventListener('input', filterWarframes);

  document.addEventListener('input', (event) => {
    if (event.target && (event.target.id === 'adminUser' || event.target.id === 'adminPass')) setTimeout(forceDarkLoginInputs, 0);
  }, true);

  window.addEventListener('resize', () => { bindControls(); optimizeImages(document, 4); });
  window.addEventListener('orientationchange', () => setTimeout(() => { bindControls(); optimizeImages(document, 4); }, 150));

  bindControls();
  preloadHomeImages(warframesData);
  optimizeImages(document, 4);
  setTimeout(() => { bindControls(); removeFakeAdminButtons(); forceDarkLoginInputs(); optimizeImages(document, 4); }, 100);
  setTimeout(() => { bindControls(); removeFakeAdminButtons(); forceDarkLoginInputs(); optimizeImages(document, 4); }, 1000);

  window.filterWarframes = filterWarframes;
  window.openBuild = openBuild;
  window.closeBuild = closeBuild;
  window.openAdminLogin = openAdminPanel;
  window.tryAdminLogin = doAdminLogin;
  window.minchOptimizeImages = optimizeImages;
  window.minchPreloadImage = preloadImage;
})();
