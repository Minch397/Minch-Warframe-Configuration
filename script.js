...
(function(){
  ...
  function minchAddShareAndStats(warframe){
    ...
    row.innerHTML = `
      <button class="minch-share-btn" type="button">Copier le lien Discord</button>
      <span class="minch-stat-pill">Vues sur cet appareil : ${Number(stats.views || 0)}</span>
    `;
    const btn = row.querySelector("button");
    btn.addEventListener("click", async () => {
      await minchCopyText(minchBuildShareUrl(warframe));
      btn.textContent = "Lien copié !";
      setTimeout(() => btn.textContent = "Copier le lien Discord", 1300);
    });
