...
/* ---------- MINCH PATCH STRICT : SAUVEGARDE STABLE PAR ID (ne touche pas à l'UI) ---------- */
(function(){
  const $minchSave = (id) => document.getElementById(id);

  function minchSaveSlug(value){
    try { return slugifyText(value || "config"); }
    catch(e){ return String(value || "config").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, ""); }
  }

  function minchMakeConfigId(item){
    const base = minchSaveSlug(item && item.name ? item.name : "config") || "config";
    return `${base}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
  }

  function minchEnsureIds(){
    const used = new Set();
    (warframesData || []).forEach((item) => {
      if (!item._id || used.has(item._id)) item._id = minchMakeConfigId(item);
      used.add(item._id);
      if (!item.builds) item.builds = [{}];
      if (!item.builds[0]) item.builds[0] = {};
    });
  }

  function minchFindIndexByIdOrName(idOrName){
    minchEnsureIds();
    let index = (warframesData || []).findIndex((w) => w._id && w._id === idOrName);
    if (index < 0) index = (warframesData || []).findIndex((w) => w.name === idOrName);
    return index;
  }

  function minchGetVal(id){
    const el = $minchSave(id);
    return el ? el.value : "";
  }

  function minchGetChecked(id){
    const el = $minchSave(id);
    return el ? !!el.checked : false;
  }

  function minchEnsureBuildShape(item){
    if (!item.builds) item.builds = [{}];
    if (!item.builds[0]) item.builds[0] = {};
    const build = item.builds[0];
    if (!build.display) build.display = {};
    if (!build.blockTexts) build.blockTexts = {};
    if (!build.specialWeapons) build.specialWeapons = { enabled: false, title: "Armes exaltées / propres à la Warframe", items: [] };
    if (!Array.isArray(build.specialWeapons.items)) build.specialWeapons.items = [];
    while (build.specialWeapons.items.length < 3) build.specialWeapons.items.push({ enabled: false, name: `Arme exaltée ${build.specialWeapons.items.length + 1}` });
    if (!build.weapons) build.weapons = {};
    ["principale", "secondaire", "melee"].forEach((slot) => {
      if (!build.weapons[slot]) build.weapons[slot] = {};
    });
  ...
  function minchFileFromInput(inputId){
    const input = $minchSave(inputId);
    return input && input.files && input.files[0] ? input.files[0] : null;
  }

  function minchImageFileToWebpDataUrl(file, maxSize = 1600, quality = 0.82){
    return new Promise((resolve) => {
      ...
      reader.onload = () => {
        const img = new Image();
        img.onerror = () => resolve(reader.result || null);
        img.onload = () => {
          ...
          catch(e){ resolve(reader.result || null); }
        };
        img.src = reader.result;
      };
      reader.readAsDataURL(file);
  ...
  async function minchStableUpload(inputId, configId, fieldKey){
    const file = minchFileFromInput(inputId);
    if (!file) return null;

    const dataUrl = await minchImageFileToWebpDataUrl(file);
    if (!dataUrl) return null;

    if (!initMinchFirebase() || !firebaseStorage) return dataUrl;

    const blob = await dataUrlToBlob(dataUrl);
    const safeConfigId = minchSaveSlug(configId || "config") || "config";
    const safeFieldKey = minchSaveSlug(fieldKey || inputId) || "image";
    const ref = firebaseStorage.ref().child(`warframe-images/configs/${safeConfigId}/${safeFieldKey}.webp`);
  ...
  async function minchSaveAllDataOnline(status){
    localStorage.setItem(ADMIN_DATA_KEY, JSON.stringify(warframesData));
    const savedOnline = await saveFirebaseAdminData();
    if (status) status.textContent = savedOnline ? "Sauvegardé en ligne sur Firebase." : "Sauvegardé localement. Firebase non disponible.";
    return savedOnline;
  }

  async function minchStrictSaveEditor(originalWarframe, isNewFromOpen){
    const saveBtn = $minchSave("saveAdminEdit");
    const status = $minchSave("adminSaveStatus");
    if (saveBtn) saveBtn.disabled = true;
    if (status) status.textContent = "Sauvegarde en cours...";
    ...
    try {
      ...
      const originalId = originalWarframe && originalWarframe._id ? originalWarframe._id : null;
      const originalName = originalWarframe && originalWarframe.name ? originalWarframe.name : null;
      const existingIndex = originalId ? minchFindIndexByIdOrName(originalId) : (originalName ? minchFindIndexByIdOrName(originalName) : -1);
      const isNew = !originalWarframe || existingIndex < 0 || isNewFromOpen === true;
      const editing = isNew ? { _id: minchMakeConfigId({ name: minchGetVal("editWarframeName") || "Nouvelle Warframe" }), name: "Nouvelle Warframe", builds: [createBuildFor("Nouvelle Warframe")] } : deepClone(warframesData[existingIndex]);

      if (!editing._id) editing._id = originalId || minchMakeConfigId(editing);
      const configId = editing._id;
      const build = minchEnsureBuildShape(editing);

      editing.name = minchGetVal("editWarframeName").trim() || "Nouvelle Warframe";
      build.name = minchGetVal("editBuildName").trim() || `${editing.name} Configuration`;
      build.infoText = minchGetVal("editInfoText").trim();
      build.video = normalizeYouTubeUrl(minchGetVal("editVideo"));
      build.skinVideo = normalizeYouTubeUrl(minchGetVal("editSkinVideo"));
      build.companionSkinVideo = normalizeYouTubeUrl(minchGetVal("editCompanionSkinVideo"));
      build.fragments = textToFragments(minchGetVal("editFragments")).slice(0, 5);

      build.blockTexts = {
        skinText: minchGetVal("textSkin"),
        configText: minchGetVal("textConfig"),
        specialText: minchGetVal("textSpecial"),
        fragmentsText: minchGetVal("textFragments"),
        primaryText: minchGetVal("textPrimary"),
        secondaryText: minchGetVal("textSecondary"),
        meleeText: minchGetVal("textMelee"),
        companionSkinText: minchGetVal("textCompanionSkin"),
        companionTextBlock: minchGetVal("textCompanion"),
        companionWeaponText: minchGetVal("textCompanionWeapon"),
        descriptionText: minchGetVal("textDescription"),
        videoExplanationText: minchGetVal("textVideoExplanation"),
        videoSkinWarframeText: minchGetVal("textVideoSkinWarframe"),
        videoSkinCompanionText: minchGetVal("textVideoSkinCompanion")
      };

      build.display = {
        skinImage: minchGetChecked("showSkinImage"), skinText: minchGetChecked("showSkinText"),
        configImage: minchGetChecked("showConfigImage"), configText: minchGetChecked("showConfigText"),
        specialWeapons: minchGetChecked("showSpecialWeapons"), specialText: minchGetChecked("showSpecialText"),
        fragments: minchGetChecked("showFragments"), fragmentsText: minchGetChecked("showFragmentsText"),
        primaryWeapon: minchGetChecked("showPrimaryWeapon"), primaryText: minchGetChecked("showPrimaryText"),
        secondaryWeapon: minchGetChecked("showSecondaryWeapon"), secondaryText: minchGetChecked("showSecondaryText"),
        meleeWeapon: minchGetChecked("showMeleeWeapon"), meleeText: minchGetChecked("showMeleeText"),
        companionSkin: minchGetChecked("showCompanionSkin"), companionSkinText: minchGetChecked("showCompanionSkinText"),
        companion: minchGetChecked("showCompanion"), companionTextBlock: minchGetChecked("showCompanionTextBlock"),
        companionWeapon: minchGetChecked("showCompanionWeapon"), companionWeaponText: minchGetChecked("showCompanionWeaponText"),
        description: minchGetChecked("showDescription"), descriptionText: minchGetChecked("showDescriptionText"),
        videoExplanation: minchGetChecked("showVideoExplanation"), videoExplanationText: minchGetChecked("showVideoExplanationText"),
        videoSkinWarframe: minchGetChecked("showVideoSkinWarframe"), videoSkinWarframeText: minchGetChecked("showVideoSkinWarframeText"),
        videoSkinCompanion: minchGetChecked("showVideoSkinCompanion"), videoSkinCompanionText: minchGetChecked("showVideoSkinCompanionText")
      };

      build.specialWeapons.title = minchGetVal("editSpecialTitle").trim() || "Armes exaltées / propres à la Warframe";
      build.specialWeapons.enabled = minchGetChecked("showSpecialWeapons");
      [0, 1, 2].forEach((i) => {
        build.specialWeapons.items[i].enabled = minchGetChecked(`editSpecialItemEnabled_${i}`);
        build.specialWeapons.items[i].name = minchGetVal(`editSpecialName_${i}`).trim() || `Arme exaltée ${i + 1}`;
      });

      ["principale", "secondaire", "melee"].forEach((slot) => {
        build.weapons[slot].name = minchGetVal(`editWeaponName_${slot}`).trim() || "Nom d'arme";
      });

      build.companion.name = minchGetVal("editCompanionName").trim() || "Nom du compagnon";
      build.companion.skinName = minchGetVal("editCompanionSkinName").trim() || "Skin du compagnon";
      build.companion.weaponName = minchGetVal("editCompanionWeaponName").trim() || "Nom de l'arme du compagnon";

      // On neutralise les anciennes échelles sans modifier le visuel actuel du site.
      delete editing.cardImageScale;
      delete editing.imageScales;
      delete build.imageScales;

      const uploads = [
        ["editCardImage", "card", (url) => editing.cardImage = url],
        ["editFashionImage", "fashion", (url) => build.fashionImage = url],
        ["editBuildImage", "warframe_build", (url) => build.buildImage = url],
        ["editSpecialImage_0", "special_1_image", (url) => build.specialWeapons.items[0].image = url],
        ["editSpecialBuild_0", "special_1_build", (url) => build.specialWeapons.items[0].buildImage = url],
        ["editSpecialImage_1", "special_2_image", (url) => build.specialWeapons.items[1].image = url],
        ["editSpecialBuild_1", "special_2_build", (url) => build.specialWeapons.items[1].buildImage = url],
        ["editSpecialImage_2", "special_3_image", (url) => build.specialWeapons.items[2].image = url],
        ["editSpecialBuild_2", "special_3_build", (url) => build.specialWeapons.items[2].buildImage = url],
        ["editWeaponImage_principale", "primary_weapon", (url) => build.weapons.principale.image = url],
        ["editWeaponBuild_principale", "primary_weapon_build", (url) => build.weapons.principale.buildImage = url],
        ["editWeaponImage_secondaire", "secondary_weapon", (url) => build.weapons.secondaire.image = url],
        ["editWeaponBuild_secondaire", "secondary_weapon_build", (url) => build.weapons.secondaire.buildImage = url],
        ["editWeaponImage_melee", "melee_weapon", (url) => build.weapons.melee.image = url],
        ["editWeaponBuild_melee", "melee_weapon_build", (url) => build.weapons.melee.buildImage = url],
        ["editCompanionSkinImage", "companion_skin", (url) => build.companion.skinImage = url],
        ["editCompanionImage", "companion", (url) => build.companion.image = url],
        ["editCompanionBuildImage", "companion_build", (url) => build.companion.buildImage = url],
        ["editCompanionWeaponImage", "companion_weapon", (url) => build.companion.weaponImage = url],
        ["editCompanionWeaponBuildImage", "companion_weapon_build", (url) => build.companion.weaponBuildImage = url]
      ];

      for (const [inputId, fieldKey, applyUpload] of uploads) {
        const url = await minchStableUpload(inputId, configId, fieldKey);
      ...
      if (isNew) {
        ...
        const indexById = minchFindIndexByIdOrName(configId);
        if (indexById >= 0) warframesData[indexById] = editing;
        else warframesData.push(editing);
      }

      minchEnsureIds();
      window.currentOpenWarframe = editing;
      await minchSaveAllDataOnline(status);
      renderGrid();
      if (document.body.classList.contains("build-open") && typeof fillBuildContent === "function") fillBuildContent(editing);
    } catch(error) {
      console.error("Erreur sauvegarde stable Minch :", error);
      if (status) status.textContent = "Erreur pendant la sauvegarde.";
    } finally {
      if (saveBtn) saveBtn.disabled = false;
    }
  ...
  const minchPreviousOpenEditor = window.openAdminEditor || (typeof openAdminEditor === "function" ? openAdminEditor : null);
  if (minchPreviousOpenEditor) {
    openAdminEditor = function(warframe){
      minchEnsureIds();
      if (warframe && !warframe._id) warframe._id = minchMakeConfigId(warframe);
      const result = minchPreviousOpenEditor(warframe);
      const isNew = !warframe;
      setTimeout(() => {
        const saveBtn = $minchSave("saveAdminEdit");
        if (saveBtn) saveBtn.onclick = (event) => {
          if (event) { event.preventDefault(); event.stopPropagation(); }
          minchStrictSaveEditor(warframe, isNew);
        };
      }, 120);
      setTimeout(() => {
        const saveBtn = $minchSave("saveAdminEdit");
  ...
  duplicateCurrentWarframe = function(){
    if (!window.currentOpenWarframe) return;
    minchEnsureIds();
    const copy = deepClone(window.currentOpenWarframe);
    copy._id = minchMakeConfigId(copy);
    copy.name = `${copy.name || "Nouvelle Warframe"} Copie`;
    if (copy.builds && copy.builds[0]) copy.builds[0].name = `${copy.name} Configuration`;
    warframesData.push(copy);
    window.currentOpenWarframe = copy;
    localStorage.setItem(ADMIN_DATA_KEY, JSON.stringify(warframesData));
    saveFirebaseAdminData().then(() => {
      renderGrid();
      if (document.body.classList.contains("build-open") && typeof fillBuildContent === "function") fillBuildContent(copy);
    });
  };
  ...
  deleteCurrentWarframe = function(){
    ...
    const ok = confirm(`Supprimer ${window.currentOpenWarframe.name} ?`);
    if (!ok) return;
    const id = window.currentOpenWarframe._id;
    const name = window.currentOpenWarframe.name;
    warframesData = (warframesData || []).filter((w) => id ? w._id !== id : w.name !== name);
    window.currentOpenWarframe = null;
    localStorage.setItem(ADMIN_DATA_KEY, JSON.stringify(warframesData));
    saveFirebaseAdminData().then(() => {
      if (typeof closeBuild === "function") closeBuild();
  ...
  const minchOldLoadFirebaseAdminData = window.loadFirebaseAdminData || (typeof loadFirebaseAdminData === "function" ? loadFirebaseAdminData : null);
  if (minchOldLoadFirebaseAdminData) {
    loadFirebaseAdminData = async function(){
      const data = await minchOldLoadFirebaseAdminData();
    ...
    window.loadFirebaseAdminData = loadFirebaseAdminData;
  }

  setTimeout(() => { minchEnsureIds(); localStorage.setItem(ADMIN_DATA_KEY, JSON.stringify(warframesData)); }, 800);
})();
