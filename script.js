...
(function(){
  ...
  const DEFAULT_DATA = {
    ...
    tierCategories: [
      {id:"warframes",title:"Warframes"},
      {id:"primary",title:"Armes principales"},
      {id:"secondary",title:"Armes secondaires"},
      {id:"melee",title:"Armes de mêlée"},
      {id:"archwing",title:"Archwing"},
      {id:"companions",title:"Compagnons"}
    ],
    tiers: [],
    weekly: [
  ...
  function normalizeData(raw){
    ...
    Object.keys(base.navCards).forEach((key)=>{
      ...
      base.navCards[key]={
        title:String(x.title||d.title), text:String(x.text||d.text), icon:String(x.icon||d.icon), image:String(x.image||""),
    ...
    base.guides = Array.isArray(raw.guides) ? raw.guides.map((x,i)=>({id:String(x?.id||`guide_${Date.now()}_${i}`),title:String(x?.title||"Guide"),text:String(x?.text||""),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1))})) : [];
    base.companions = Array.isArray(raw.companions) ? raw.companions.map((x,i)=>({id:String(x?.id||`companion_${Date.now()}_${i}`),title:String(x?.title||"Compagnon"),text:String(x?.text||""),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1))})) : [];
    ...
    if(Array.isArray(raw.tierCategories)&&raw.tierCategories.length) base.tierCategories=raw.tierCategories.map((x,i)=>({id:String(x?.id||`cat_${i}`),title:String(x?.title||`Catégorie ${i+1}`)}));
    base.tiers = Array.isArray(raw.tiers) ? raw.tiers.map((x,i)=>({id:String(x?.id||`tier_${Date.now()}_${i}`),category:String(x?.category||"melee"),title:String(x?.title||"Tier List"),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||3))})) : [];
    base.weekly = Array.isArray(raw.weekly) ? raw.weekly.map((x,i)=>({id:String(x?.id||`weekly_${Date.now()}_${i}`),title:String(x?.title||"Tâche hebdomadaire"),width:Math.min(3,Math.max(1,Number(x?.width)||1))})) : base.weekly;
  ...
  const viewIds = {home:"mainHub",warframes:"warframeSection",guides:"guidesSection",tiers:"tiersSection",weekly:"weeklySection",companions:"companionsSection"};
  ...
  function openGuideEditor(item){
    ...
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title:title||"Guide",text,image,width};
  ...
  function openCompanionEditor(item){
    ...
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title:title||"Compagnon",text,image,width};
  ...
  function openTierEditor(item){
    ...
    save.onclick=async()=>{
      ...
      const next={id:existing.id,category:existing.category||currentTierCategory||"melee",title,image,width};
  ...
  function openTaskEditor(item){
    ...
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title,width}; const i=hubData.weekly.findIndex(x=>x.id===existing.id); if(i>=0)hubData.weekly[i]=next; else hubData.weekly.push(next);
      await saveHub(); close();
    };
  }
  ...
  function renderTierCategories(){
    ...
    if(intro){const d=hubData.tierIntro||{};intro.style.fontSize=`${Number(d.size)||16}px`;intro.style.textAlign=d.align||"center";intro.innerHTML=`${d.text?`<p>${esc(d.text).replace(/\n/g,"<br>")}</p>`:""}${window.isAdminMode?`<button class="hub-admin-btn" type="button" data-tier-intro-edit>${d.text?"Modifier le texte":"＋ Ajouter un texte"}</button>`:""}`;intro.querySelector("[data-tier-intro-edit]")?.addEventListener("click",openTierIntroEditor);}
    const cats=document.getElementById("tierCategoriesGrid");if(!cats)return;
    cats.innerHTML=(hubData.tierCategories||[]).map(c=>`<button class="tier-category-card" type="button" data-tier-category="${esc(c.id)}"><span>${esc(c.title)}</span></button>`).join("");
    cats.querySelectorAll("[data-tier-category]").forEach(b=>b.onclick=()=>openTierCategory(b.dataset.tierCategory));
  }
  ...
  function renderWeekly(){
    ...
    if(info) info.textContent=`Prochaine réinitialisation : lundi ${next.toLocaleDateString("fr-FR")} à 02:00`;
    root.innerHTML=hubData.weekly.map((x,i)=>`<div class="weekly-task ${checks[x.id]?"is-done":""}" data-id="${esc(x.id)}" data-card-width="${Math.min(3,Math.max(1,Number(x.width)||1))}"><input id="weekly_${i}" type="checkbox" ${checks[x.id]?"checked":""}><label for="weekly_${i}">${esc(x.title)}</label>${window.isAdminMode?`<button class="hub-admin-btn" data-task-edit>Modifier</button><button class="hub-admin-btn danger" data-task-delete>×</button>`:""}</div>`).join("")+(window.isAdminMode?`<button class="hub-add-card" type="button" data-task-add>＋ Ajouter une tâche hebdomadaire</button>`:"");
  ...
  function renderAllHubContent(){renderHubCards();renderGuides();renderCompanions();renderTiers();renderWeekly();renderFooter();}
