...
(function(){
  ...
  function normalizeData(raw){
    ...
    base.tiers = Array.isArray(raw.tiers) ? raw.tiers.map((x,i)=>({id:String(x?.id||`tier_${Date.now()}_${i}`),title:String(x?.title||"Tier List"),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1))})) : [];
  ...
  function openHubCardEditor(key){
    ...
    save.onclick=async()=>{
      ...
      hubData.navCards[key]={
        icon:String(overlay.querySelector("#hubCardIcon").value||"").trim()||d.icon,
  ...
  function renderHubCards(){
    document.querySelectorAll(".hub-card[data-hub-card-key]").forEach((card)=>{
      ...
      if(icon)icon.textContent=d.icon;if(title)title.textContent=d.title;if(text)text.textContent=d.text;
  ...
  function openTierEditor(item){
    const existing=item||{id:`tier_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:"",image:"",width:1};
    const inputId=`hubTierFile_${Date.now()}`;
    const {overlay,close,save}=editorShell(item?"Modifier la Tier List":"Ajouter une Tier List",`<label>Titre<input id="hubTierTitle" class="admin-input" value="${esc(existing.title)}"></label><label>Largeur de la bulle<select id="hubTierWidth" class="admin-input"><option value="1" ${Number(existing.width||1)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||1)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||1)===3?"selected":""}>3 colonnes</option></select></label><label>Image<input id="${inputId}" class="admin-input" type="file" accept="image/*"></label><label>ou URL de l'image<input id="hubTierUrl" class="admin-input" value="${esc(existing.image)}" placeholder="https://..."></label>`);
    save.onclick=async()=>{
      const title=String(overlay.querySelector("#hubTierTitle").value||"").trim()||"Tier List";
      let image=String(overlay.querySelector("#hubTierUrl").value||"").trim();
      const fileInput=overlay.querySelector(`#${inputId}`);
      if(fileInput?.files?.[0] && typeof uploadImageInput==="function"){
        save.disabled=true; save.textContent="Envoi...";
        const uploaded=await uploadImageInput(inputId,"tier_lists");
        if(uploaded) image=uploaded;
      }
      const width=Math.min(3,Math.max(1,Number(overlay.querySelector("#hubTierWidth")?.value)||1));
      const next={id:existing.id,title,image,width};
  ...
  function renderTiers(){
    ...
    root.innerHTML=hubData.tiers.map(x=>`<article class="tier-card" data-id="${esc(x.id)}" data-card-width="${Math.min(3,Math.max(1,Number(x.width)||1))}"><h3>${esc(x.title)}</h3>${x.image?`<img src="${esc(x.image)}" alt="${esc(x.title)}" loading="lazy">`:`<div class="weekly-reset-info">Image non ajoutée</div>`}${window.isAdminMode?`<div class="hub-admin-actions"><button class="hub-admin-btn" data-tier-edit>Modifier</button><button class="hub-admin-btn danger" data-tier-delete>Supprimer</button></div>`:""}</article>`).join("")+(window.isAdminMode?`<button class="hub-add-card" type="button" data-tier-add>＋ Ajouter une Tier List</button>`:hubData.tiers.length?"":"<div class='tier-card'><h3>Aucune Tier List pour le moment</h3></div>");
