...
function ensureWarframeInternalId(warframe){
  if(!warframe.__id){
    warframe.__id = 'wf_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,8);
  }
  return warframe.__id;
}

function ensureUniqueWarframeName(name){
  const base = String(name || 'Nouvelle Warframe').trim() || 'Nouvelle Warframe';
  let candidate = base;
  let i = 2;
  while(warframesData.some(w => w.name === candidate)){
    candidate = `${base} ${i}`;
    i++;
  }
  return candidate;
...
async function finalSaveEditor(editing, isNew, originalName){
    ...
    try {
      const build = sectionDefaults(getOrCreateBuild(editing));
      ensureWarframeInternalId(editing);
      const requestedName = val("editWarframeName").trim() || "Nouvelle Warframe";
      editing.name = isNew ? ensureUniqueWarframeName(requestedName) : requestedName;
      ...
      if (isNew) {
        ...
        const editId = ensureWarframeInternalId(editing);
        let index = warframesData.findIndex(w => w.__id === editId);
        if (index < 0) index = warframesData.findIndex(w => w.name === originalName);
