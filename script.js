...
(function(){
  ...
  function renderEidolonCycle(){
    ...
    root.innerHTML=`<div class="eidolon-cycle-content"><strong>${esc(mainText)}</strong><span class="eidolon-time">${fmtDuration(remaining).replace(/^0j /,'')}</span><small>${esc(subText)}</small></div>${window.isAdminMode?`<button class="hub-admin-btn activity-bg-edit" type="button" data-eidolon-bg>Modifier</button>`:""}`;
  ...
  function injectV53SafeStyles(){
    if(document.getElementById("v53SafeStyles"))return;const st=document.createElement("style");st.id="v53SafeStyles";st.textContent=`
      ...
      #weeklySection .weekly-task{min-width:0;box-sizing:border-box;overflow:hidden;width:auto!important;max-width:none!important;writing-mode:horizontal-tb!important}#weeklySection .weekly-task[data-card-width="1"]{grid-column:span 1}#weeklySection .weekly-task[data-card-width="2"]{grid-column:span 2}#weeklySection .weekly-task[data-card-width="3"]{grid-column:1 / -1}#weeklySection .weekly-task label{min-width:0;white-space:normal!important;overflow-wrap:break-word!important;word-break:normal!important;writing-mode:horizontal-tb!important}
...
(function initFeaturedCarousel(){
  ...
  function maxScroll(){return Math.max(0,root.scrollWidth-root.clientWidth)}
  ...
  loadLocal();render();resetFeatured();loadOnline();
