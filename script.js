...
(function(){
  ...
  function controlsLayout(){
    ...
    if (isBuild) {
      [adminBtn, musicPanel].forEach(el => {
        if (!el) return;
    ...
    if (adminBtn) {
      setImportant(adminBtn, 'position', 'absolute');
      setImportant(adminBtn, 'top', '16px');
      setImportant(adminBtn, 'right', '16px');
      setImportant(adminBtn, 'z-index', '2200');
      setImportant(adminBtn, 'opacity', '1');
      setImportant(adminBtn, 'visibility', 'visible');
      setImportant(adminBtn, 'pointer-events', 'auto');
      setImportant(adminBtn, 'transform', 'none');
    }
    if (musicPanel) {
      setImportant(musicPanel, 'position', 'absolute');
      setImportant(musicPanel, 'top', '16px');
      setImportant(musicPanel, 'left', '16px');
      setImportant(musicPanel, 'z-index', '2200');
      setImportant(musicPanel, 'opacity', '1');
      setImportant(musicPanel, 'visibility', 'visible');
      setImportant(musicPanel, 'pointer-events', 'auto');
      setImportant(musicPanel, 'transform', 'none');
    }
  }
    ...
    modal.querySelectorAll('input[type="file"]').forEach(input => {
      ...
      if (input.dataset.minchPreviewPlus !== 'true') {
        ...
        input.addEventListener('change', function(){
          ...
          preview.innerHTML = `<img src="${url}" alt="preview"><div class="minch-zone-file">${file.name}</div><div class="minch-zone-subtitle">Clique ou dépose une autre image pour remplacer</div>`;
        });
      }
    });
  }
