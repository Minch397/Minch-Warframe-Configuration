...
/* ---------- MINCH V5 : FORMATAGE RICHE DES ZONES DE TEXTE ---------- */
(function(){
  ...
  function sanitizeRichHTML(value){
    ...
    if (!/[<>]/.test(raw)) return escapeRich(raw).replace(/\n/g,"<br>");

    const allowed = new Set(["B","STRONG","I","EM","SPAN","BR","DIV","P"]);
    ...
    function rebuild(node){
      ...
      node.childNodes.forEach(child => {
        if (child.nodeType === Node.TEXT_NODE) { out += escapeRich(child.nodeValue || ""); return; }
        if (child.nodeType !== Node.ELEMENT_NODE) return;
        const tag = child.tagName;
        if (!allowed.has(tag)) { out += escapeRich(child.textContent || ""); return; }
        ...
        if (tag === "SPAN") {
          ...
          const safeColor = /^(#[0-9a-f]{3,8}|rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)|rgba\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*(0|1|0?\.\d+)\s*\)|[a-z]{3,20})$/i.test(color) ? color : "";
          out += safeColor ? `<span style="color:${safeColor}">${inner}</span>` : inner;
          return;
        }
        if (tag === "DIV" || tag === "P") { out += `<div>${inner}</div>`; return; }
      });
      return out;
    }
  ...
  function sourceToEditorHTML(value){
    const raw = String(value || "");
    return /<(strong|b|em|i|span|br|div|p)(\s|>|\/)/i.test(raw)
      ? sanitizeRichHTML(raw)
      : escapeRich(raw).replace(/\n/g,"<br>");
  }
  ...
  function restoreSelection(editor){
    ...
    if (!sel || !editor._minchRange) return;
    sel.removeAllRanges();
  ...
  function exec(editor, textarea, command, value=null){
    editor.focus();
  ...
  function enhanceTextarea(textarea){
    ...
    toolbar.innerHTML = `
      <button type="button" class="minch-rich-btn minch-rich-bold" title="Gras"><strong>B</strong></button>
      <button type="button" class="minch-rich-btn minch-rich-italic" title="Italique"><em>I</em></button>
      <span class="minch-rich-sep"></span>
      <button type="button" class="minch-rich-color-swatch" data-color="#ffffff" style="--swatch:#ffffff" title="Blanc"></button>
      <button type="button" class="minch-rich-color-swatch" data-color="#7dd3fc" style="--swatch:#7dd3fc" title="Bleu"></button>
      <button type="button" class="minch-rich-color-swatch" data-color="#c084fc" style="--swatch:#c084fc" title="Violet"></button>
      <button type="button" class="minch-rich-color-swatch" data-color="#fb7185" style="--swatch:#fb7185" title="Rouge"></button>
      <button type="button" class="minch-rich-color-swatch" data-color="#facc15" style="--swatch:#facc15" title="Jaune"></button>
      <button type="button" class="minch-rich-color-swatch" data-color="#86efac" style="--swatch:#86efac" title="Vert"></button>
      <label class="minch-rich-custom-color" title="Autre couleur">A<input type="color" value="#7dd3fc"></label>
    `;
    ...
    ["keyup","mouseup","focus"].forEach(type => editor.addEventListener(type, () => saveSelection(editor)));
    editor.addEventListener("input", () => { saveSelection(editor); syncRichEditor(editor, textarea); });

    toolbar.addEventListener("mousedown", e => { if (e.target.closest("button,label")) e.preventDefault(); });
    toolbar.querySelector(".minch-rich-bold").onclick = () => exec(editor, textarea, "bold");
    toolbar.querySelector(".minch-rich-italic").onclick = () => exec(editor, textarea, "italic");
    toolbar.querySelectorAll(".minch-rich-color-swatch").forEach(btn => {
      btn.onclick = () => exec(editor, textarea, "foreColor", btn.dataset.color);
    });
    const picker = toolbar.querySelector('input[type="color"]');
    picker.addEventListener("input", () => exec(editor, textarea, "foreColor", picker.value));
    picker.addEventListener("change", () => exec(editor, textarea, "foreColor", picker.value));

    syncRichEditor(editor, textarea);
  }

  function scan(root=document){
    if (root.matches && root.matches("textarea.admin-textarea")) enhanceTextarea(root);
    if (root.querySelectorAll) root.querySelectorAll("textarea.admin-textarea").forEach(enhanceTextarea);
  }

  const observer = new MutationObserver(mutations => {
    mutations.forEach(m => m.addedNodes.forEach(node => { if (node.nodeType === 1) scan(node); }));
  });
