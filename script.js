...
/* ---------- PATCH PROPRE MINCH : retour config + œil mdp + actions admin robustes ---------- */
(function(){
  function byId(id){ return document.getElementById(id); }

  function ensureConfigBack(){
    let btn = byId('minchConfigBack');
    if (!btn) {
      btn = document.createElement('button');
      btn.id = 'minchConfigBack';
      btn.type = 'button';
    ...
    btn.style.display = document.body.classList.contains('build-open') ? 'flex' : 'none';
    return btn;
  }

  function setAdminConnectedView(){
    ...
    if (window.isAdminMode) {
      ...
      if (submit) {
        ...
        submit.onclick = function(e){
          ...
          setTimeout(setAdminConnectedView, 30);
        };
      }
    }
  }
  ...
  function setupPasswordEye(){
    ...
    eye.classList.toggle('visible', isVisible);
  ...
  function bindAdminModalBacks(){
    document.querySelectorAll('#adminLoginModal .admin-back').forEach(function(btn){
      btn.onclick = function(e){
        if (e) { e.preventDefault(); e.stopPropagation(); }
        if (typeof closeAdminLogin === 'function') closeAdminLogin();
      };
    });
    document.querySelectorAll('#adminEditorModal .admin-back').forEach(function(btn){
      btn.onclick = function(e){
        if (e) { e.preventDefault(); e.stopPropagation(); }
  ...
  function bindBuildActionButtons(){
    document.querySelectorAll('[data-admin-build-action]').forEach(function(btn){
      btn.onclick = function(e){
        if (e) { e.preventDefault(); e.stopPropagation(); }
        const action = btn.getAttribute('data-admin-build-action');
        if (action === 'edit' && window.currentOpenWarframe && typeof openAdminEditor === 'function') openAdminEditor(window.currentOpenWarframe);
        if (action === 'duplicate' && typeof duplicateCurrentWarframe === 'function') duplicateCurrentWarframe();
        if (action === 'delete' && typeof deleteCurrentWarframe === 'function') deleteCurrentWarframe();
      };
    });
  }
  ...
  function bindAll(){
    ...
    setupPasswordEye();
    bindAdminModalBacks();
    bindBuildActionButtons();
    if (byId('adminLoginModal') && byId('adminLoginModal').classList.contains('open')) setAdminConnectedView();
  }

  const prevOpenBuild = window.openBuild || (typeof openBuild === 'function' ? openBuild : null);
  if (prevOpenBuild) {
    openBuild = function(w){
      const result = prevOpenBuild(w);
      setTimeout(bindAll, 0);
      setTimeout(bindAll, 150);
      setTimeout(bindAll, 750);
      return result;
    };
    window.openBuild = openBuild;
  }

  const prevCloseBuild = window.closeBuild || (typeof closeBuild === 'function' ? closeBuild : null);
  if (prevCloseBuild) {
    closeBuild = function(){
      const result = prevCloseBuild();
      setTimeout(bindAll, 0);
      return result;
    };
  ...
  const prevFillBuildContent = window.fillBuildContent || (typeof fillBuildContent === 'function' ? fillBuildContent : null);
  if (prevFillBuildContent) {
    fillBuildContent = function(w){
      const result = prevFillBuildContent(w);
      setTimeout(bindBuildActionButtons, 0);
      setTimeout(ensureConfigBack, 0);
      return result;
    };
    window.fillBuildContent = fillBuildContent;
  }

  const prevOpenAdminLogin = window.openAdminLogin || (typeof openAdminLogin === 'function' ? openAdminLogin : null);
  if (prevOpenAdminLogin) {
    openAdminLogin = function(){
      const result = prevOpenAdminLogin();
      setTimeout(function(){ setAdminConnectedView(); setupPasswordEye(); bindAdminModalBacks(); }, 0);
      setTimeout(function(){ setAdminConnectedView(); setupPasswordEye(); bindAdminModalBacks(); }, 80);
      return result;
    };
  ...
  document.addEventListener('click', function(e){
    const actionBtn = e.target.closest && e.target.closest('[data-admin-build-action]');
    if (actionBtn) {
    ...
    const eye = e.target.closest && e.target.closest('#adminPasswordToggle');
    if (eye) {
      setupPasswordEye();
    }
  }, true);
  ...
  window.addEventListener('orientationchange', function(){ setTimeout(bindAll, 100); });
  setTimeout(bindAll, 0);
