...
/* ---------- PATCH MINCH V8 : évite QuotaExceeded localStorage ---------- */
(function(){
  const ADMIN_DATA_KEY_SAFE = typeof ADMIN_DATA_KEY !== 'undefined' ? ADMIN_DATA_KEY : 'minch_warframes_data';
  const BIG_DATA_URL_LIMIT = 1800;

  function isBigDataUrl(value){
    return typeof value === 'string' && value.startsWith('data:image/') && value.length > BIG_DATA_URL_LIMIT;
  }
  ...
  function stripHeavyImages(value){
    ...
    Object.keys(value).forEach(function(key){
      ...
      if (isBigDataUrl(item)) {
        // localStorage est trop petit pour garder les images base64.
        // On garde la config texte, et on évite de casser toute la sauvegarde.
        clone[key] = '';
        clone[key + 'MissingReason'] = 'Image non sauvegardée localement : Firebase Storage non disponible ou quota navigateur dépassé.';
      } else {
        clone[key] = stripHeavyImages(item);
      }
    });
    return clone;
  }
  ...
  function showQuotaWarningOnce(){
    if (window.__minchQuotaWarningShown) return;
    window.__minchQuotaWarningShown = true;
    console.warn('[Minch] localStorage plein : les grosses images base64 ne peuvent pas être gardées localement. Active Firebase Storage pour les sauvegarder en ligne.');

    const status = document.getElementById('adminSaveStatus');
    if (status) {
      status.textContent = 'Config sauvegardée, mais les images sont trop lourdes pour le stockage local. Active Firebase Storage pour les garder en ligne.';
    }
  }

  function safeSaveAdminData(){
    try {
      localStorage.setItem(ADMIN_DATA_KEY_SAFE, JSON.stringify(warframesData));
      return true;
    } catch(error) {
      if (error && (error.name === 'QuotaExceededError' || String(error).includes('quota'))) {
        const lightData = stripHeavyImages(warframesData);
        try {
          localStorage.setItem(ADMIN_DATA_KEY_SAFE, JSON.stringify(lightData));
          showQuotaWarningOnce();
          return false;
        } catch(secondError) {
          console.error('[Minch] Même la sauvegarde allégée a échoué :', secondError);
          showQuotaWarningOnce();
          return false;
        }
      }
  ...
  // Remplace la fonction globale utilisée par refreshAfterAdminChange / duplication / sauvegarde.
  try {
    saveAdminData = safeSaveAdminData;
    window.saveAdminData = safeSaveAdminData;
  } catch(e) {}

  // Rend la compression locale plus agressive quand Firebase Storage n'est pas prêt.
  if (typeof minchCompressImageFile === 'function' && !minchCompressImageFile.__minchQuotaV8) {
    const oldCompress = minchCompressImageFile;
    minchCompressImageFile = function(file, options){
      const saferOptions = Object.assign({ maxSize: 900, quality: 0.58 }, options || {});
      return oldCompress(file, saferOptions);
    };
    minchCompressImageFile.__minchQuotaV8 = true;
    window.minchCompressImageFile = minchCompressImageFile;
  }
})();
