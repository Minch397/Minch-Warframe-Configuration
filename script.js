...
(function(){
  ...
  function normalizeData(raw){
    ...
    base.guides = Array.isArray(raw.guides) ? raw.guides.map((x,i)=>({id:String(x?.id||`guide_${Date.now()}_${i}`),title:String(x?.title||"Guide"),text:String(x?.text||""),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1)),updatedAt:Number(x?.updatedAt)||0})) : [];
    base.companions = Array.isArray(raw.companions) ? raw.companions.map((x,i)=>({id:String(x?.id||`companion_${Date.now()}_${i}`),title:String(x?.title||"Compagnon"),text:String(x?.text||""),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1)),updatedAt:Number(x?.updatedAt)||0})) : [];
    base.elements = Array.isArray(raw.elements) ? raw.elements.map((x,i)=>({id:String(x?.id||`element_${Date.now()}_${i}`),title:String(x?.title||"Élément / Statut"),text:String(x?.text||""),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||1)),updatedAt:Number(x?.updatedAt)||0})) : [];
    ...
    base.tiers = Array.isArray(raw.tiers) ? raw.tiers.map((x,i)=>({id:String(x?.id||`tier_${Date.now()}_${i}`),category:String(x?.category||"melee"),title:String(x?.title||"Tier List"),image:String(x?.image||""),width:Math.min(3,Math.max(1,Number(x?.width)||3)),updatedAt:Number(x?.updatedAt)||0})) : [];
    base.weekly = Array.isArray(raw.weekly) ? raw.weekly.map((x,i)=>({id:String(x?.id||`weekly_${Date.now()}_${i}`),title:String(x?.title||"Tâche hebdomadaire"),width:Math.min(3,Math.max(1,Number(x?.width)||1)),updatedAt:Number(x?.updatedAt)||0})) : base.weekly;
  ...
  function collectRecentUpdates(){
    const out=[];
    (window.warframesData||warframesData||[]).forEach(w=>{const t=Number(w?._updatedAt)||0;if(t)out.push({type:"warframe",id:String(w?._id||w?.name||""),title:String(w?.name||"Configuration Warframe"),updatedAt:t});});
    (hubData.guides||[]).forEach(x=>{if(Number(x.updatedAt))out.push({type:"guides",id:x.id,title:x.title,updatedAt:Number(x.updatedAt)});});
    (hubData.companions||[]).forEach(x=>{if(Number(x.updatedAt))out.push({type:"companions",id:x.id,title:x.title,updatedAt:Number(x.updatedAt)});});
    (hubData.elements||[]).forEach(x=>{if(Number(x.updatedAt))out.push({type:"elements",id:x.id,title:x.title,updatedAt:Number(x.updatedAt)});});
    (hubData.tiers||[]).forEach(x=>{if(Number(x.updatedAt))out.push({type:"tier",id:x.id,category:x.category,title:x.title,updatedAt:Number(x.updatedAt)});});
    (hubData.weekly||[]).forEach(x=>{if(Number(x.updatedAt))out.push({type:"weekly",id:x.id,title:x.title,updatedAt:Number(x.updatedAt)});});
  ...
  function renderRecentUpdates(){
    ...
    root.querySelectorAll("[data-recent-index]").forEach(b=>b.addEventListener("click",()=>openRecentUpdate(items[Number(b.dataset.recentIndex)])));
  }
  ...
  function openGuideEditor(item){
    const existing=item||{id:`guide_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:"",text:"",image:"",width:1};
    const inputId=`hubGuideFile_${Date.now()}`;
    const {overlay,close,save}=editorShell(item?"Modifier le guide":"Ajouter un guide",`<label>Titre<input id="hubGuideTitle" class="admin-input" value="${esc(existing.title)}"></label><label>Texte<textarea id="hubGuideText" class="admin-textarea">${esc(existing.text)}</textarea></label><label>Largeur de la bulle<select id="hubGuideWidth" class="admin-input"><option value="1" ${Number(existing.width||1)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||1)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||1)===3?"selected":""}>3 colonnes</option></select></label><label>Image<input id="${inputId}" class="admin-input" type="file" accept="image/*"></label><label>ou URL de l'image<input id="hubGuideImage" class="admin-input" value="${esc(existing.image||"")}" placeholder="https://..."></label>`);
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title:title||"Guide",text,image,width,updatedAt:Date.now()};
  ...
  function openCompanionEditor(item){
    const existing=item||{id:`companion_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:"",text:"",image:"",width:1};
    const inputId=`hubCompanionFile_${Date.now()}`;
    const {overlay,close,save}=editorShell(item?"Modifier le compagnon":"Ajouter un compagnon",`<label>Nom du compagnon<input id="hubCompanionTitle" class="admin-input" value="${esc(existing.title)}"></label><label>Pourquoi le choisir ?<textarea id="hubCompanionText" class="admin-textarea">${esc(existing.text)}</textarea></label><label>Largeur de la bulle<select id="hubCompanionWidth" class="admin-input"><option value="1" ${Number(existing.width||1)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||1)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||1)===3?"selected":""}>3 colonnes</option></select></label><label>Image du compagnon<input id="${inputId}" class="admin-input" type="file" accept="image/*"></label><label>ou URL de l'image<input id="hubCompanionImage" class="admin-input" value="${esc(existing.image||"")}" placeholder="https://..."></label>`);
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title:title||"Compagnon",text,image,width,updatedAt:Date.now()};
  ...
  function openElementEditor(item){
    const existing=item||{id:`element_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:"",text:"",image:"",width:1,updatedAt:0};
    const inputId=`hubElementFile_${Date.now()}`;
    const {overlay,close,save}=editorShell(item?"Modifier l’élément / statut":"Ajouter un élément / statut",`<label>Titre<input id="hubElementTitle" class="admin-input" value="${esc(existing.title)}" placeholder="Ex. Viral, Corrosif, Grineer..."></label><label>Explication<textarea id="hubElementText" class="admin-textarea">${esc(existing.text)}</textarea></label><label>Largeur de la bulle<select id="hubElementWidth" class="admin-input"><option value="1" ${Number(existing.width||1)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||1)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||1)===3?"selected":""}>3 colonnes</option></select></label><label>Image / tableau<input id="${inputId}" class="admin-input" type="file" accept="image/*"></label><label>ou URL de l'image<input id="hubElementImage" class="admin-input" value="${esc(existing.image||"")}" placeholder="https://..."></label>`);
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title:title||"Élément / Statut",text,image,width,updatedAt:Date.now()};
  ...
  function openTierEditor(item){
    const existing=item||{id:`tier_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,category:currentTierCategory||"melee",title:"",image:"",width:3};
    const inputId=`hubTierImg_${Date.now()}`;
    const {overlay,close,save}=editorShell(item?"Modifier le bloc":"Ajouter un bloc Tier",`<label>Tier / Titre<input id="hubTierTitle" class="admin-input" value="${esc(existing.title)}" placeholder="Ex. Tier S"></label><label>Largeur du bloc<select id="hubTierWidth" class="admin-input"><option value="1" ${Number(existing.width||3)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||3)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||3)===3?"selected":""}>Toute la largeur</option></select></label><label>Image / Screen<input id="${inputId}" class="admin-input" type="file" accept="image/*"></label><label>ou URL de l'image<input id="hubTierUrl" class="admin-input" value="${esc(existing.image)}" placeholder="https://..."></label>`);
    save.onclick=async()=>{
      ...
      const next={id:existing.id,category:existing.category||currentTierCategory||"melee",title,image,width,updatedAt:Date.now()};
  ...
  function openTaskEditor(item){
    const existing=item||{id:`weekly_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,title:"",width:1};
    const {overlay,close,save}=editorShell(item?"Modifier la tâche":"Ajouter une tâche",`<label>Nom de la tâche<input id="hubTaskTitle" class="admin-input" value="${esc(existing.title)}"></label><label>Largeur de la bulle<select id="hubTaskWidth" class="admin-input"><option value="1" ${Number(existing.width||1)===1?"selected":""}>1 colonne</option><option value="2" ${Number(existing.width||1)===2?"selected":""}>2 colonnes</option><option value="3" ${Number(existing.width||1)===3?"selected":""}>3 colonnes</option></select></label>`);
    save.onclick=async()=>{
      ...
      const next={id:existing.id,title,width,updatedAt:Date.now()}; const i=hubData.weekly.findIndex(x=>x.id===existing.id); if(i>=0)hubData.weekly[i]=next; else hubData.weekly.push(next);
      await saveHub(); close();
    };
  }
