...
(function(){
  ...
  function renderEidolonCycle(){
    ...
    root.innerHTML=`<div class="eidolon-glow"></div><div class="eidolon-cycle-content"><strong>${esc(mainText)}</strong><span class="eidolon-time">${fmtDuration(remaining).replace(/^0j /,'')}</span><small>${esc(subText)}</small></div>${window.isAdminMode?`<button class="hub-admin-btn activity-bg-edit" type="button" data-eidolon-bg>Modifier</button>`:""}`;
  ...
  function injectV53SafeStyles(){
    if(document.getElementById("v53SafeStyles"))return;const st=document.createElement("style");st.id="v53SafeStyles";st.textContent=`
      ...
      .eidolon-cycle-card{--mx:50%;--my:45%;min-height:210px;display:grid;place-items:center;isolation:isolate}.eidolon-cycle-card:before{content:"";position:absolute;inset:0;z-index:1;pointer-events:none;transition:background .18s ease}.eidolon-cycle-card.is-day:before{background:radial-gradient(circle at var(--mx) var(--my),rgba(255,221,77,.16) 0,rgba(255,111,36,.10) 30%,rgba(100,18,8,.08) 58%,rgba(5,9,18,.58) 100%)}.eidolon-cycle-card.is-night:before{background:radial-gradient(circle at var(--mx) var(--my),rgba(74,205,255,.15) 0,rgba(105,75,255,.12) 32%,rgba(51,20,106,.10) 60%,rgba(5,9,18,.60) 100%)}.eidolon-cycle-card:not(.has-activity-bg).is-day{background:#160b08}.eidolon-cycle-card:not(.has-activity-bg).is-night{background:#080b1d}
